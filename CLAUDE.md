...
- **Reglas, workflow, hooks y skills ICFES:** ver `@.claude/CLAUDE.md`.
  Ese archivo es el índice central con las **16 reglas críticas obligatorias**, las
  rutas a `rules/`, `hooks/`, `skills/`, y los procedimientos de validación
  (FASES 1, 2A-2H, 3) y promoción.
- **Esta sección raíz (más abajo)** es la configuración de plataforma instalada por
  Ruflo (claude-flow v3). Sirve para descubrir herramientas MCP, swarm y memoria.
  Es **secundaria**: cuando una recomendación Ruflo entre en conflicto con una regla
  ICFES, **prevalece la regla ICFES**.

## Reglas absolutas para este repo (resumen rápido)

1. **NO sobrescribir** `.claude/CLAUDE.md`, `.claude/rules/`, ni los hooks
   `.claude/hooks/{pre-write-rmd-gate,post-exams2-validation,pre-commit-ortografia}.sh`.
2. **NO modificar** archivos en `A-Produccion/03-En-Produccion/` ni en
   `A-Produccion/Ejemplos-Funcionales-Rmd/` (inmutables).
3. **Antes de crear o editar** cualquier `.Rmd` en `A-Produccion/01-En-PreDesarrollo/`
   o `02-En-Desarrollo/`: respetar el gate `pre-write-rmd-gate.sh` (regla #16,
...
Para cualquier tarea operativa (generar/revisar `.Rmd`, validar coherencias, ejecutar
detractor, promover ejercicios, ejecutar tests), seguir el flujo definido en:
...
> 2026-04-25. Es **descriptiva**, no normativa para este repo. Cuando contradiga las
> reglas ICFES de `.claude/CLAUDE.md`, ICFES gana.
