...
2. Si similitud < 95%: Ejecutar `/iterar python` para refinar
3. Si similitud ≥ 95%: Ejecutar `/generar-r` para continuar con R
...
- Próximo paso: Ejecutar `/generar-tikz` para iniciar generación TikZ

### Si fase_actual es "tikz_iteracion":
- Próximo paso: Ejecutar `/comparar tikz` para evaluar iteración actual
- Si similitud < 95%: Ejecutar `/iterar tikz`
- Si similitud ≥ 95%: Ejecutar `/generar-python`

### Si fase_actual es "tikz_validado":
- Próximo paso: Ejecutar `/generar-python` para continuar con Python

### Si fase_actual es "python_iteracion":
- Próximo paso: Ejecutar `/comparar python` para evaluar iteración actual
- Si similitud < 95%: Ejecutar `/iterar python`
- Si similitud ≥ 95%: Ejecutar `/generar-r`

### Si fase_actual es "python_validado":
- Próximo paso: Ejecutar `/generar-r` para continuar con R

### Si fase_actual es "r_iteracion":
- Próximo paso: Ejecutar `/comparar r` para evaluar iteración actual
- Si similitud < 95%: Ejecutar `/iterar r`
- Si similitud ≥ 95%: Ejecutar `/exportar`

### Si fase_actual es "r_validado" o "completado":
- Próximo paso: Ejecutar `/exportar` para generar reporte final
