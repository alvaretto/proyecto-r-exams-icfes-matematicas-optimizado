# Changelog del índice `.claude/CLAUDE.md` (v2.6 → actual)

> Este archivo **no se importa con `@`** desde `.claude/CLAUDE.md`: existe para que el historial
> no viaje en el contexto de cada sesión y de cada subagente. Historial anterior (v2.2 a v2.5):
> `.claude/docs/CHANGELOG.md`.
>
> **Convención desde la v3.25.0**: toda entrada de cambios se escribe AQUÍ, arriba del todo, con el
...
> commits `d5e9a76c` y `dd3d59d1`. Tres lecciones nuevas, todas medidas sobre el mismo artefacto.

- **Regla #22 v1.8 — nueva §P7-F**: enumerar el espacio de diseño **antes** de añadir una segunda
  rama. Medido: de las cuatro combinaciones *teorema × magnitud pedida*, **sólo una** admite clave
  única **y** ≥1 distractor de su mismo molde — y es, exactamente, la que eligió el ICFES
  (Seno-pide-lado tiene **6 claves**). Cuando la enumeración demuestra que no hay rama
  alternativa, el canal es **estructural**: corresponde un override registrado, no una pasada de
  corrección. Nota añadida a §P7-E: una regla relacional con **aplicabilidad 0,0 %** es cobertura
  **nominal**, peor que declarar la familia no aplicable.
- **Regla #9 v1.5 — alcance de la auditoría**: el detractor audita también los **overrides
  firmados y las invariantes locales** del subproyecto, no sólo el `.Rmd`. Caso medido: un override
  registraba **+4,4 pp** obtenido con una batería **ciega** al canal que ese mismo override
  aceptaba; con la sonda incluida, la cifra real era **+18,8 pp** y el veredicto `BLOQUEA`.
- **Regla #24 v1.1 — H-4 reforzada**: `mapa-paginas.md` y el OCR apuntaban **los dos** al ítem
  equivocado, porque ambos derivan del mismo mapeo; el pie impreso decía 20 y el mapa, 19. Dos
  fuentes derivadas coincidiendo **no son confirmación**: sólo el número impreso en el escaneo lo es.
- **Errores 35, 36 y 37** en `patrones-errores-conocidos.md`: segunda clave cuando el criterio del
  enunciado admite dos lecturas (28 de 200 versiones) · una invariante en prosa no aborta nada
  (**0/60** al revertirla → **52/60** y **60/60** tras cablearla) · un cero que no prueba nada
  (cobertura nominal y sondas sin control positivo).
- **Índice**: dos filas nuevas en la tabla de referencia rápida. 20 044 → 20 564 bytes, sobre un
  presupuesto de ~60 000.

### Cambios v3.26.0 (2026-09-08)

> Fases 2 y 3 del plan de costos del 2026-09-07. Antes de cablear nada se midió qué proveedores
> respondían de verdad: **Codex** (ChatGPT) sin cuota hasta el 19-09-2026 (el error invita a
> «Upgrade to Plus»); **Ollama Cloud** sin créditos, tanto por API key como por la cuenta de
> `ollama signin` («requires a subscription or extra usage»); **OmniRoute** con Mistral en 429
> permanente y su clave GLM sin saldo; **DeepSeek** directo con **9,31 USD** de saldo prepagado; y la
> suscripción de Claude (Opus 5 y Sonnet 5 vía `claude-cli`) respondiendo en 6 s. El plan original
> ponía a Codex como segundo detractor y a Ollama Cloud como tier barato: se reordenó sobre lo que
> existe, dejando la cascada preparada para cuando esos proveedores vuelvan.

- **FASE 2 — OpenClaw** (`~/.openclaw/openclaw.json`, todo vía `openclaw config set`; copia previa
  en `~/.openclaw/backups/openclaw.json.orig-20260908-fase2` y en el directorio de reversión):
  primario `claude-cli/claude-opus-5` (era Fable 5.1, el modelo más caro del catálogo) con respaldo
  `claude-cli/claude-sonnet-5`; alias `fable`, `opus`, `sonnet` (`/model fable` recupera Fable a
  demanda); subagentes por defecto `deepseek/deepseek-v4-flash` con respaldo Sonnet 5;
  `utilityModel` y `compaction.model` en DeepSeek V4 Flash; `modelPolicy.allow` gana `deepseek/*` y
  `openai/*`; plugin `@openclaw/deepseek-provider` con la clave como SecretRef de archivo
  (`~/.openclaw/secrets/deepseek.key`, 0600, extraída de `~/.credenciales` sin imprimirla);
  embeddings de memoria en Ollama local (`nomic-embed-text`, índice reconstruido: 16 archivos,
  50 chunks, 768 dims) en vez de OpenAI sin billing. **Pendiente de `openclaw gateway restart`**
  para cargar el plugin DeepSeek y el proveedor de embeddings; hasta entonces las rutas
  `deepseek/*` fallan a través del gateway y el respaldo Sonnet 5 absorbe.
- **FASE 3 — segundo detractor heterogéneo**: `.claude/scripts/detractor-hetero.sh` (cascada
  `deepseek → ollama-cloud → codex → local`, mismo prompt de sistema que `AgenteDetractor`, solo
  lectura, reporte con marcador junto al ejercicio) + comando `/detractor-hetero` + sección nueva
  y tercera fila de la tabla de detractores en `.claude/rules/detractor-obligatorio.md` (v1.4).
  Complementa la FASE 2C, **no la sustituye ni sella `detractor_fase2c`**. Regla #9 del índice
  actualizada.
- **Reversión**: `bash .claude/backups/2026-09-08-fase2-3/revertir.sh` (restaura índice, regla,
  changelog y `openclaw.json`; borra script y comando; no toca la fase 1). Tarball previo de
  `.claude/`: `.claude.pre-fase2-3-20260908-062659.tar.gz` (ignorado por git, patrón
  `.claude.pre-*.tar.gz`). Verificación: `verificar.sh` en el mismo directorio.
- **Cierre de la fase 1**: el `verificar.sh` del 2026-09-07 se ejecutó — copia idéntica a
  `git HEAD`, 0 importaciones `@`, 25 reglas, invariantes I-1..I-10 en verde (214 tests, 0 fallos).
- **No aplicado, documentado**: (a) desactivar Ruflo/agent teams — los invariantes I-3/I-7 y el
  ADR 001 protegen explícitamente la convivencia Ruflo+ICFES; requiere decisión aparte. (b) Los
  alias de `~/.zshrc` `claude-glm5`, `claude-deepseek-cloud` y `claude-minimax` apuntan a modelos
  retirados de Ollama Cloud (`glm-5`, `deepseek-v3.2`, `minimax-m2.5`). (c) Login OAuth de OpenAI
  en OpenClaw (`openclaw models auth login --provider openai --device-code`) cuando Codex recupere
  cuota el 19-09; `modelPolicy.allow` ya lo admite.

### Cambios v3.25.0 (2026-09-07)

> El índice pesaba **1435 líneas, ~120 KB, ~60 000 tokens**, y **1275 de esas líneas eran
> historial**. Además importaba con `@` unos 35 archivos —entre ellos `REGLAS_CRITICAS.md`, que a
> su vez importa las reglas—, así que cada sesión **y cada subagente** arrancaban con cientos de
> miles de tokens antes del primer prompt. Ninguna de esas piezas era un gate: los gates son hooks
> y tests, y no dependen de que un documento esté en contexto.

- **RECORTE DE CONTEXTO (1/2) — el changelog sale del índice.** Las entradas v2.6 → v3.24.0 se
  mueven íntegras a este archivo. El índice conserva las 25 reglas con su resumen operativo, los
  comandos, la estructura del repositorio y las referencias rápidas.
- **RECORTE DE CONTEXTO (2/2) — los `@` pasan a rutas planas.** Cada documento se lee bajo demanda
  con `Read` cuando la tarea lo requiere. Nueva sección «Política de contexto» en el índice con el
  presupuesto (~15 000 tokens) y la regla de que el changelog vive aquí.
- **TRES AJUSTES GLOBALES, fuera del repo** (`~/.claude/settings.json`): `model` `opus[1m]` →
  `opus`; `effortLevel` `xhigh` → `high`; `fastMode` `true` → `false`. Motivo, del informe de
  costos del 2026-09-07: el modo rápido se cobra a tarifa doble contra créditos de uso (Opus 5:
  10/50 USD por millón frente a 5/25), la ventana de 1M habilita el tramo «long context» que
  `/usage` marca como comportamiento caro, y la latencia de este pipeline la dominan los renders de
  R, no el modelo. Para una tarea puntual que lo justifique (detractor, arquitectura): lanzar con
  `claude --effort xhigh` o cambiar el modelo en la sesión con `/model opus[1m]`.
- **REVERSIBLE con un comando**: `bash .claude/backups/2026-09-07-recorte-contexto/revertir.sh`
  restaura el índice anterior íntegro y el `settings.json` global, y elimina este archivo. Copia
  íntegra del índice en ese directorio; copia del `settings.json` junto al original
  (`~/.claude/settings.json.orig-20260907-recorte`). `verificar.sh` comprueba tamaño, ausencia de
  `@`, conteo de reglas, fidelidad de la copia frente a `git HEAD` e invariantes I-1..I-10.
- **NO VERIFICADO en esta pasada, declarado**: los tests de invariantes no se ejecutaron porque la
  sesión (OpenClaw, Fable 5.1) no tenía shell aprobado; tampoco se creó el tarball de la regla #17.
  I-2 exige ≥ 17 reglas numeradas e identidad ICFES: ambas se conservan por construcción (25 reglas,
  mismo título). Pendiente al reabrir el repo: `verificar.sh` y `/context` en sesión nueva para
  medir el tamaño real del prompt.
- **LÍMITE DECLARADO**: `REGLAS_CRITICAS.md` sigue importando reglas con `@` en su interior; es
  inocuo mientras nadie lo vuelva a importar desde el índice. Los hooks Ruflo de
  `.claude/settings.json` (routeo en cada prompt, importación de memoria al arrancar) y
  `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` quedan fuera de esta pasada.
