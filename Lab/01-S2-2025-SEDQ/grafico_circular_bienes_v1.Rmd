...
# Crear figura con tamaño controlado - Ajustar sin leyenda
...
patches, texts, autotexts = plt.pie(
    ...
    autopct='%d%%',
    pctdistance=0.6,  # Más cerca del centro para evitar solapamiento
    wedgeprops={'edgecolor': 'white', 'linewidth': 1},
    textprops={'fontsize': 9}  # Tamaño de fuente para las etiquetas
)

# Crear etiquetas más visibles para cada sector
# Definir nombres cortos para las etiquetas (para mostrar en los sectores)
# Usamos las mismas etiquetas que ya están definidas en 'labels'

# Añadir etiquetas directamente en los sectores
for i, p in enumerate(patches):
    # Obtener el ángulo medio del sector en radianes
    ang = (p.theta1 + p.theta2)/2
    # Convertir a radianes
    ang_rad = np.pi * ang/180

    # Calcular la posición para las etiquetas (más cerca del borde)
    # Factor más grande para posicionar más lejos del centro
    x = 0.75 * np.cos(ang_rad)
    y = 0.75 * np.sin(ang_rad)

    # Añadir recuadros con las etiquetas cortas directamente en los sectores
    # Estos reemplazarán a los porcentajes que ya están en el gráfico
    bbox_props = {'boxstyle': 'round,pad=0.3',
                 'facecolor': 'white',
                 'edgecolor': 'black',
                 'alpha': 0.9}  # Mayor opacidad

    # Determinar la alineación horizontal según la posición
    ha = 'center'
    va = 'center'

    # Añadir la etiqueta con un recuadro blanco
    plt.text(x, y, labels[i],
            ...
            va=va,
            bbox=bbox_props,
            zorder=10)  # Mayor zorder para asegurar que esté encima

# Configuración estética de los textos de porcentaje con recuadros
for autotext in autotexts:
    autotext.set_fontsize(9)  # Tamaño de fuente más pequeño
    autotext.set_weight('bold')
    autotext.set_color('white')
    # Crear un recuadro semitransparente detrás del texto
    # Configurar recuadro con propiedades
    bbox_props = {'boxstyle': 'round,pad=0.2',  # Padding más pequeño
                 'facecolor': 'black',
                 'alpha': 0.7,  # Mayor opacidad para mejor contraste
                 'edgecolor': 'none'}
    autotext.set_bbox(bbox_props)

# Ya no necesitamos leyenda separada porque tenemos etiquetas en los sectores
plt.axis('equal')  # Asegurar que el gráfico sea un círculo
# Eliminamos plt.legend() ya que tenemos etiquetas en los sectores

# Eliminar el título de la parte superior y añadirlo en la parte inferior
# plt.title() ya no se usa aquí
...
# Ajustar los márgenes para el título inferior
# [izquierda, inferior, derecha, superior]
plt.tight_layout(pad=1.5, rect=[0, 0.05, 1, 1])
...
Si `r personas_bien1_bien3` `r termino_personas` de la `r contexto` poseen `r bien1` y `r bien3`, ¿cuántas personas poseen solo `r bien3`?
