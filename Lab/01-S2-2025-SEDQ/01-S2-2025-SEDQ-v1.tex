...
\CommentTok{\# Obtener la ruta temporal para guardar la imagen}
\NormalTok{temp\_dir }\OtherTok{\textless{}{-}} \FunctionTok{tempdir}\NormalTok{()}
\NormalTok{grafico\_path }\OtherTok{\textless{}{-}} \FunctionTok{file.path}\NormalTok{(temp\_dir, }\StringTok{"grafico\_circular"}\NormalTok{)}
...
\StringTok{\# Guardar el gráfico en múltiples formatos para asegurar compatibilidad}
\StringTok{plt.savefig("\%s.png", dpi=150, bbox\_inches="tight", format="png")}
\StringTok{plt.savefig("\%s.pdf", dpi=150, bbox\_inches="tight", format="pdf")}
\StringTok{plt.close()}
...
\NormalTok{grafico\_path, grafico\_path}
\NormalTok{)}

\CommentTok{\# Ejecutar el código Python}
\FunctionTok{py\_run\_string}\NormalTok{(codigo\_python\_grafico)}

\CommentTok{\# Copiar los archivos a la carpeta actual para que estén disponibles}
\FunctionTok{file.copy}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(grafico\_path, }\StringTok{".png"}\NormalTok{), }\StringTok{"grafico\_circular.png"}\NormalTok{, }\AttributeTok{overwrite =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Verificar si se generó el archivo PDF}
\ControlFlowTok{if}\NormalTok{ (}\FunctionTok{file.exists}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(grafico\_path, }\StringTok{".pdf"}\NormalTok{))) \{}
  \FunctionTok{file.copy}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(grafico\_path, }\StringTok{".pdf"}\NormalTok{), }\StringTok{"grafico\_circular.pdf"}\NormalTok{, }\AttributeTok{overwrite =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{\} }\ControlFlowTok{else}\NormalTok{ \{}
  \CommentTok{\# Si no se generó el PDF, convertir el PNG a PDF usando magick}
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{requireNamespace}\NormalTok{(}\StringTok{"magick"}\NormalTok{, }\AttributeTok{quietly =} \ConstantTok{TRUE}\NormalTok{)) \{}
\NormalTok{    img }\OtherTok{\textless{}{-}}\NormalTok{ magick}\SpecialCharTok{::}\FunctionTok{image\_read}\NormalTok{(}\StringTok{"grafico\_circular.png"}\NormalTok{)}
\NormalTok{    magick}\SpecialCharTok{::}\FunctionTok{image\_write}\NormalTok{(img, }\StringTok{"grafico\_circular.pdf"}\NormalTok{, }\AttributeTok{format =} \StringTok{"pdf"}\NormalTok{)}
    \FunctionTok{message}\NormalTok{(}\StringTok{"Convertido PNG a PDF usando magick"}\NormalTok{)}
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
    \FunctionTok{warning}\NormalTok{(}\StringTok{"No se pudo generar el archivo PDF del gráfico. Se usará PNG en su lugar."}\NormalTok{)}
\NormalTok{  \}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
Se realizó un(a) estudio a un grupo de personas de una organización
tecnológica sobre el tipo de bienes que poseen. Los resultados se
presentan en la gráfica.

\includegraphics[width=0.75\linewidth]{grafico_circular.pdf}

Si 180 personas de el(la) organización poseen coche y(e) piso, ¿cuántas
personas poseen solo piso?
...
Paso 1: Calcular el número total de personas en la organización. Si 180
personas representan el 18\% del total, entonces:
...
personas que tienen solo piso es 34\% del total, por lo tanto:
