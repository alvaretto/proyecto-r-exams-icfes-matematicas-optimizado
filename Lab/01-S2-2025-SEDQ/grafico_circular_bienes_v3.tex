...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Valor conocido para el cálculo: Número de personas con bien2 (incluye solo bien2 y bien1+bien2)}
\NormalTok{personas\_bien2\_valores }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\DecValTok{72}\NormalTok{, }\DecValTok{96}\NormalTok{, }\DecValTok{120}\NormalTok{, }\DecValTok{144}\NormalTok{, }\DecValTok{168}\NormalTok{, }\DecValTok{192}\NormalTok{, }\DecValTok{216}\NormalTok{, }\DecValTok{240}\NormalTok{)}
\NormalTok{personas\_bien2 }\OtherTok{\textless{}{-}} \FunctionTok{sample}\NormalTok{(personas\_bien2\_valores, }\DecValTok{1}\NormalTok{)}

\CommentTok{\# Calcular el total de personas}
\CommentTok{\# personas\_bien2 representa la suma de personas\_solo\_bien2 y personas\_bien1\_bien2}
\CommentTok{\# Esto es igual a (p\_solo\_bien2 + p\_bien1\_bien2)\% del total}
\NormalTok{total\_personas }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(personas\_bien2 }\SpecialCharTok{*} \DecValTok{100} \SpecialCharTok{/}\NormalTok{ (p\_solo\_bien2 }\SpecialCharTok{+}\NormalTok{ p\_bien1\_bien2))}
...
\NormalTok{personas\_bien1\_bien3 }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(total\_personas }\SpecialCharTok{*}\NormalTok{ p\_bien1\_bien3 }\SpecialCharTok{/} \DecValTok{100}\NormalTok{)}

\CommentTok{\# Verificar que personas\_bien2 es aproximadamente igual a la suma de solo\_bien2 y bien1\_bien2}
\CommentTok{\# Si hay diferencia, ajustar personas\_bien1\_bien2 para mantener la coherencia}
\NormalTok{personas\_bien2\_calculado }\OtherTok{\textless{}{-}}\NormalTok{ personas\_solo\_bien2 }\SpecialCharTok{+}\NormalTok{ personas\_bien1\_bien2}
\ControlFlowTok{if}\NormalTok{ (personas\_bien2\_calculado }\SpecialCharTok{!=}\NormalTok{ personas\_bien2) \{}
  \CommentTok{\# Ajustar personas\_bien1\_bien2 para que la suma sea exactamente personas\_bien2}
\NormalTok{  personas\_bien1\_bien2 }\OtherTok{\textless{}{-}}\NormalTok{ personas\_bien2 }\SpecialCharTok{{-}}\NormalTok{ personas\_solo\_bien2}
\NormalTok{\}}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Recalcular el total de personas para asegurar coherencia}
\NormalTok{total\_personas\_recalculado }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(personas)}
...
\FunctionTok{test\_that}\NormalTok{(}\StringTok{"La suma de todas las categorías es igual al total"}\NormalTok{, \{}
  \FunctionTok{expect\_equal}\NormalTok{(}\FunctionTok{sum}\NormalTok{(personas), total\_personas\_recalculado)}
\NormalTok{\})}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Actualizar el total de personas para mantener coherencia}
\NormalTok{total\_personas }\OtherTok{\textless{}{-}}\NormalTok{ total\_personas\_recalculado}
...
\CommentTok{\# La respuesta correcta es el número de personas con bien3 y bien1}
...
\NormalTok{distractor1 }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(personas\_bien2 }\SpecialCharTok{/} \DecValTok{3}\NormalTok{)  }\CommentTok{\# Un tercio del valor dado en el problema}
...
\NormalTok{distractor3 }\OtherTok{\textless{}{-}}\NormalTok{ personas\_bien2  }\CommentTok{\# El mismo valor dado en el problema}
...
Se realizó un(a) estudio a un grupo de colaboradores de un(a) grupo
empresarial sobre el tipo de bienes que poseen. Los(las) cifras se
presentan en la gráfica.

\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{grafico_circular.png}

Si 120 colaboradores de el(la) grupo empresarial poseen domicilio,
¿cuántas personas poseen departamento y coche?
...
\tightlist
\item
  Sabemos que 120 colaboradores poseen domicilio.
\item
  Según el gráfico circular, las personas que poseen domicilio se
  dividen en dos grupos:

  \begin{itemize}
  \tightlist
  \item
    Las que poseen solo domicilio: 29\% del total
  \item
    Las que poseen coche y domicilio: 16\% del total
  \end{itemize}
\item
  Por lo tanto, el porcentaje total de personas con domicilio es 45\%.
\item
  También observamos en el gráfico que las personas que poseen coche y
  departamento representan el 30\% del total.
\end{itemize}

\subsubsection{Paso 2: Calcular el número total de
personas}\label{paso-2-calcular-el-nuxfamero-total-de-personas}

Para encontrar el total de colaboradores en la grupo empresarial,
utilizamos la siguiente relación de proporcionalidad:

Si 45\% del total = 120 colaboradores
...
Por lo tanto, el total de colaboradores en la grupo empresarial es 267.

\subsubsection{Paso 3: Calcular el número de personas que poseen
departamento y
coche}\label{paso-3-calcular-el-nuxfamero-de-personas-que-poseen-departamento-y-coche}

Una vez conocido el total, podemos calcular cuántas personas poseen
departamento y coche utilizando el porcentaje correspondiente del
gráfico circular:

Si 100\% del total = 267 colaboradores.

Entonces 30\% del total = Y colaboradores
...
\tightlist
\item
  coche y departamento: 80 colaboradores (30\% del total)
\item
  Solo departamento: 43 colaboradores (16\% del total)
\item
  Solo domicilio: 77 colaboradores (29\% del total)
\item
  Solo coche: 24 colaboradores (9\% del total)
\item
  coche y domicilio: 43 colaboradores (16\% del total).
\end{itemize}

La suma de todas estas categorías es 267 colaboradores, que coincide con
nuestro total calculado de 267 colaboradores.

\subsubsection{Conclusión}\label{conclusiuxf3n}

Por lo tanto, 80 colaboradores de la grupo empresarial poseen
departamento y coche.
