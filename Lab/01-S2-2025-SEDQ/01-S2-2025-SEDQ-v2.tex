...
\CommentTok{\# La respuesta correcta es el número de personas que poseen solo bien\_transporte (carro)}
\NormalTok{respuesta\_correcta }\OtherTok{\textless{}{-}}\NormalTok{ total\_solo\_bien\_transporte}

\CommentTok{\# Generar distractores plausibles}
\NormalTok{distractor1 }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(total\_bien\_transporte\_y\_vivienda1 }\SpecialCharTok{*}\NormalTok{ p\_solo\_bien\_transporte }\SpecialCharTok{/}\NormalTok{ p\_bien\_transporte\_y\_vivienda1)}
\NormalTok{distractor2 }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(total\_personas }\SpecialCharTok{*}\NormalTok{ p\_solo\_bien\_transporte }\SpecialCharTok{/}\NormalTok{ p\_bien\_transporte\_y\_vivienda1)}
\NormalTok{distractor3 }\OtherTok{\textless{}{-}}\NormalTok{ total\_personas }\SpecialCharTok{{-}}\NormalTok{ total\_solo\_bien\_transporte}
...
Se realizó un(a) análisis a un grupo de personas de una organización
alimentaria sobre el tipo de bienes que poseen. Los resultados se
presentan en la gráfica.

\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{grafico_circular} 

}

\end{figure}

Si 48 personas de el(la) organización poseen coche y(e) casa, ¿cuántas
personas poseen solo coche?
...
\tightlist
\item
  21\% de las personas tienen coche y casa
\item
  6\% de las personas tienen solo coche (este es el dato que buscamos)
\item
  Sabemos que 48 personas tienen coche y casa
\end{itemize}

Paso 1: Calcular el número total de personas en la organización. Si 48
personas representan el 21\% del total, entonces:

\[\text{Total de personas} = \frac{\text{Personas con coche y casa} \times 100\%}{\text{Porcentaje de personas con coche y casa}}\]

\[\text{Total de personas} = \frac{48 \times 100\%}{21\%} = 229\]

Paso 2: Calcular cuántas personas tienen solo coche. El porcentaje de
personas que tienen solo coche es 6\% del total, por lo tanto:

\[\text{Personas con solo coche} = \text{Total de personas} \times \frac{\text{Porcentaje de personas con solo coche}}{100\%}\]

\[\text{Personas con solo coche} = 229 \times \frac{6\%}{100\%} = 14\]

Por lo tanto, 14 personas poseen solo coche.
...
exsolution: 0100 exshuffle: TRUE exsection: Interpretación de
