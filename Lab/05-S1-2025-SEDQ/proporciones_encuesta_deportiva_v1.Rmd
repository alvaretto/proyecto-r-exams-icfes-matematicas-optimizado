...
# POOL DE DISTRACTORES DESAFIANTES - Seleccionar 3 aleatoriamente
distractores_disponibles <- list()

# Distractor tipo 1: Fracción reducida equivalente (MUY DESAFIANTE)
distractores_disponibles[[1]] <- paste0("alrededor de ", numerador_reducido_fmt, " de cada ", denominador_reducido_fmt, " ",
                                        termino_usuarios, " del ", contexto, " da por favorito al ", equipo_correcto, ".")

# Distractor tipo 2: Confunde muestra con población usando el equipo correcto
distractores_disponibles[[2]] <- paste0("alrededor de ", valor_correcto_fmt, " de cada ", poblacion_total_fmt, " ",
                                        termino_usuarios, " del ", contexto, " da por favorito al ", equipo_correcto, ".")

# Distractor tipo 3: Usa fracción reducida pero con población total (DOBLE CONFUSIÓN)
distractores_disponibles[[3]] <- paste0("alrededor de ", numerador_reducido_fmt, " de cada ", poblacion_total_fmt, " ",
                                        termino_usuarios, " del ", contexto, " da por favorito al ", equipo_correcto, ".")

# Distractor tipo 4: Afirma que ninguno prefiere un equipo no incluido en la muestra
distractores_disponibles[[4]] <- paste0("ninguno de los ", poblacion_total_fmt, " ", termino_usuarios,
                                        " del ", contexto, " da por favorito al ",
                                        sample(setdiff(equipos_disponibles, equipos_seleccionados), 1), ".")

# Distractor tipo 5: Confunde tamaño de muestra con preferencias totales
distractores_disponibles[[5]] <- paste0("sólo ", tamano_muestra_fmt, " de los ", poblacion_total_fmt, " ",
                                        termino_usuarios, " del ", contexto,
                                        " tienen preferencia por un equipo para ganar la ", competicion, ".")

# Distractor tipo 6: Usa proporción incorrecta con otro equipo de la muestra
...
distractores_disponibles[[6]] <- paste0("alrededor de ", otro_valor_fmt, " de cada ", tamano_muestra_fmt, " ",
                                        termino_usuarios, " del ", contexto, " da por favorito al ", otro_equipo, ".")

# Distractor tipo 7: Fracción reducida con otro equipo (TRIPLE CONFUSIÓN)
...
distractores_disponibles[[7]] <- paste0("alrededor de ", num_otro_red, " de cada ", den_otro_red, " ",
                                        termino_usuarios, " del ", contexto, " da por favorito al ", otro_equipo, ".")

# SELECCIÓN INTELIGENTE DE DISTRACTORES
# Asegurar que siempre se incluya el distractor de fracción reducida (tipo 1) si es diferente de la respuesta correcta
distractores_seleccionados <- list()

# Verificar si la fracción reducida es diferente de la original (para evitar duplicados)
if (numerador_reducido != valor_correcto || denominador_reducido != tamano_muestra) {
  # Incluir SIEMPRE el distractor de fracción reducida como primer distractor
  distractores_seleccionados[[1]] <- distractores_disponibles[[1]]
  # Seleccionar 2 distractores adicionales de los restantes (excluyendo el tipo 1)
  indices_restantes <- sample(2:7, 2)
  distractores_seleccionados[[2]] <- distractores_disponibles[[indices_restantes[1]]]
  distractores_seleccionados[[3]] <- distractores_disponibles[[indices_restantes[2]]]
} else {
  # Si la fracción ya está reducida, seleccionar 3 distractores diferentes
  indices_seleccionados <- sample(2:7, 3)
  distractores_seleccionados[[1]] <- distractores_disponibles[[indices_seleccionados[1]]]
  distractores_seleccionados[[2]] <- distractores_disponibles[[indices_seleccionados[2]]]
  distractores_seleccionados[[3]] <- distractores_disponibles[[indices_seleccionados[3]]]
}

# Crear vector final de opciones y mezclar
opciones <- c(respuesta_correcta, distractores_seleccionados[[1]],
              distractores_seleccionados[[2]], distractores_seleccionados[[3]])
...
# VALIDACIONES DE INTEGRIDAD PARA DISTRACTORES
# Verificar que no hay opciones duplicadas
if (length(unique(opciones)) != 4) {
  stop("Error: Se generaron opciones duplicadas. Opciones: ", paste(opciones, collapse=" | "))
}

# Verificar que la respuesta correcta está incluida
if (!(respuesta_correcta %in% opciones)) {
  stop("Error: La respuesta correcta no está en las opciones finales")
}
