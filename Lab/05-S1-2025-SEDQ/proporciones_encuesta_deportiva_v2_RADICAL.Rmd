...
- **Error conceptual muestra-población**: El distractor que menciona proporciones sobre la población total de `r poblacion_total_fmt` `r termino_usuarios` es incorrecto, ya que `r articulo_encuesta` `r termino_encuesta` solo se realizó a `r tamano_muestra_fmt` personas.

- **Error de interpretación de porcentajes**: El distractor que confunde el porcentaje (`r round(valor_correcto/tamano_muestra*100, 1)`%) con el valor absoluto (`r valor_correcto_fmt`) comete un error conceptual básico.

- **Error de cálculo matemático**: El distractor que usa un valor incorrecto (`r valor_incorrecto_fmt`) no corresponde a los datos reales del gráfico para el `r equipo_correcto`.
