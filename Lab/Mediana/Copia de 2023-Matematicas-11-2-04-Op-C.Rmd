...
Answerlist
----------
* El `r modelos[3]` tiene una mediana igual a la del `r modelos[2]` y menor que la del `r modelos[1]`.
* El `r modelos[3]` tiene una mediana mayor que la del `r modelos[2]` y que la del `r modelos[1]`.
* El `r modelos[3]` tiene una mediana igual a la del `r modelos[1]` y menor que la del `r modelos[2]`.
* El `r modelos[3]` tiene una mediana mayor que la del `r modelos[1]` y menor que la del `r modelos[2]`.

Solution
========
La respuesta correcta es: El `r modelos[3]` tiene una mediana igual a la del `r modelos[1]` y menor que la del `r modelos[2]`.

Para resolver este problema, debemos:

1. Ordenar los datos de cada modelo y encontrar su mediana:

* `r modelos[1]`: `r sort(matriz_datos[1,])` → Mediana = `r med1`
* `r modelos[2]`: `r sort(matriz_datos[2,])` → Mediana = `r med2`
* `r modelos[3]`: `r sort(matriz_datos[3,])` → Mediana = `r med3`
...
Por lo tanto, el `r modelos[3]` tiene una mediana igual a la del `r modelos[1]` y menor que la del `r modelos[2]`.
