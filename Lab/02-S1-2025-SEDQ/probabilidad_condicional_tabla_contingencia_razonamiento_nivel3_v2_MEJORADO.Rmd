...
test_that("Función de escape LaTeX funciona correctamente", {
  ...
  # Usar fixed = TRUE para búsqueda literal de secuencias LaTeX
  expect_true(grepl("\\'{a}", texto_escapado, fixed = TRUE))
  expect_true(grepl("\\~{n}", texto_escapado, fixed = TRUE))
  expect_false(grepl("á", texto_escapado, fixed = TRUE))
  expect_false(grepl("ñ", texto_escapado, fixed = TRUE))

  # Verificar que el texto contiene las secuencias de escape esperadas
  expect_true(nchar(texto_escapado) > nchar(texto_prueba))
  expect_true(grepl("\\\\", texto_escapado, fixed = TRUE))
})
...
test_that("Renderizado LaTeX es seguro y correcto", {
  ...
  # Verificar que los textos escapados contienen las secuencias LaTeX correctas
  if(grepl("á", pregunta_seleccionada$texto_evento, fixed = TRUE)) {
    expect_true(grepl("\\'{a}", texto_evento_latex_safe, fixed = TRUE))
  }
  if(grepl("ñ", pregunta_seleccionada$texto_evento, fixed = TRUE)) {
    expect_true(grepl("\\~{n}", texto_evento_latex_safe, fixed = TRUE))
  }
  if(grepl("á", pregunta_seleccionada$texto_condicion, fixed = TRUE)) {
    expect_true(grepl("\\'{a}", texto_condicion_latex_safe, fixed = TRUE))
  }
  if(grepl("ñ", pregunta_seleccionada$texto_condicion, fixed = TRUE)) {
    expect_true(grepl("\\~{n}", texto_condicion_latex_safe, fixed = TRUE))
  }
})
...
test_that("Corrección de errores de expresiones regulares", {
  ...
  # Verificar que las búsquedas con fixed=TRUE funcionan
  resultado_acentos <- escapar_latex(texto_con_acentos)
  resultado_enie <- escapar_latex(texto_con_enie)

  expect_silent(grepl("\\'{a}", resultado_acentos, fixed = TRUE))
  expect_silent(grepl("\\~{n}", resultado_enie, fixed = TRUE))
