...
# R-exams Expertise
- Expert in creating ICFES mathematics exercises using R-exams with LaTeX/Sweave format (.Rnw), metadata, data generation functions (300+ unique versions), diversity tests, advanced randomization, TikZ diagrams, and complete .Rnw structure following RepositorioMatematicasICFES_R_Exams project standards.
...
- For R-exams ICFES exercises: ALWAYS consult functional examples in '/home/pequeniomanjaro/Documentos/proyecto-r-exams-icfes-matematicas-optimizado/Auxiliares/Ejemplos-Funcionales-Rmd/Rnw/' before generating, correcting, or optimizing any .Rnw file - these examples contain proven solutions from countless error corrections and code optimizations.
...
- Master task plan for generating/correcting ICFES R-exams exercises: 6 phases (Analysis+Functional Examples, Technical Configuration, Data Generation+300 versions, Visualizations, Content, Validation+Testing) with 21 specific subtasks following User Guidelines and mandatory functional examples, including mandatory web research for updated official ICFES theoretical information, TikZ graphics prioritization, and comprehensive quality criteria for mathematics exercise development following RepositorioMatematicasICFES_R_Exams standards.
- User prefers Spanish language responses when working with ICFES R-exams exercises and wants to focus using the TEMPLATE_Plan_Tareas_ICFES_R_Exams_Rnw.md task template.
...
- User requested creation of an advanced methodology for correcting recurring errors in R-exams .Rnw files, similar to the advanced TikZ strategy, as these errors happen frequently across various .Rnw files in the project.
...
- Key error patterns identified: gender concordance ("La conteo" → "El conteo"), incorrect table-text order in TikZ, duplicate answer options, missing LaTeX packages, incomplete meta-information.
- Methodology documented in three files: METODOLOGIA_Correccion_Errores_Recurrentes_ICFES_R_Exams.md, BIBLIOTECA_Soluciones_Errores_Comunes.md, CHECKLIST_Validacion_Archivos_Rnw.md.
...
- User requested reorganization of TEMPLATE_Plan_Tareas_ICFES_R_Exams_Rnw.md to prioritize TikZ methodology at the beginning instead of at the end.
...
- Implemented TikZ prioritization: "In case of graphics (even non-mathematical ones), generate with TikZ code, prioritizing it over other alternatives, applying the 'ADVANCED TIKZ METHODOLOGY' section."
...
- Template now starts with image analysis and TikZ methodology as the primary workflow, with other alternatives only when TikZ is not viable.
...
- User requested implementation of an automated conditional system in TEMPLATE_Plan_Tareas_ICFES_R_Exams_Rnw.md that detects graphical content (charts, tables, diagrams) in PNG images and activates specialized TikZ replication workflows with 98%+ visual fidelity.
...
# Protocolo Anti-Errores de Implementación ICFES R-exams (2025-01-29)
- User identified critical implementation errors in TikZ code generation (variable interpolation, syntax mixing, extra chunks) and requested systematic prevention strategies to avoid future errors.
- Created comprehensive anti-error protocol with 7 key strategies: 1) Mandatory consultation of functional examples BEFORE writing code, 2) Step-by-step validation with incremental compilation, 3) Automatic error correction methodology during implementation (not just at end), 4) Self-verification protocol with mandatory checklist, 5) Strict priority order (examples first, copy patterns, adapt carefully), 6) Golden rule "if not in functional examples don't improvise", 7) Critical alert signals for complex interpolation and R-LaTeX mixing.
- Updated TEMPLATE_Plan_Tareas_ICFES_R_Exams_Rnw.md with new PHASE 1.5 "PROTOCOLO ANTI-ERRORES DE IMPLEMENTACIÓN" including pre-implementation checklist, continuous validation during implementation, critical alert signals, and final self-verification protocol.
- Enhanced PHASE 7 with new section 7.0 "VALIDACIÓN CONTINUA DURANTE IMPLEMENTACIÓN" emphasizing prevention over correction, chunk-by-chunk verification, and immediate error correction during phases 3-6.
- Added comprehensive "ANEXO: PROTOCOLO ANTI-ERRORES DE IMPLEMENTACIÓN" with complete prevention protocol, continuous validation checklist, critical alert signals, final mandatory checklist, most common errors to avoid, and error recovery protocol.
- Identified specific error patterns: incorrect variable interpolation in TikZ, extra chunks, mixed R-LaTeX syntax without proven patterns, invented configurations not in functional examples.
- Protocol emphasizes "PREVENTION > CORRECTION" philosophy with mandatory functional examples consultation, exact pattern copying, continuous validation, and strict adherence to proven solutions.
- Updated all memories and documentation files to reflect new anti-error protocol integration with existing methodologies (Sistema Condicional Automático, TikZ Advanced, Error Correction).
- Template now includes 1195+ lines with complete error prevention integration while maintaining all original functionality and adding systematic quality assurance throughout implementation process.