...
icfes:
  ...
  tarea: Identificar la expresión que representa el cálculo del ahorro en un descuento porcentual.
...
# Definir fracciones invertidas para cada porcentaje (para opciones incorrectas)
fraccion_invertida <- switch(as.character(porcentaje_descuento),
                           "10" = "\\frac{10}{1}",
                           "12" = "\\frac{25}{3}",
                           "15" = "\\frac{20}{3}",
                           "18" = "\\frac{50}{9}",
...
# Elegir aleatoriamente cuál opción (A, B o C) será la correcta
indice_correcto <- sample(1:3, 1)

# Generar opciones según el índice correcto
# Opción A
if (indice_correcto == 1) {
  # Opción A correcta: Fracción correcta × precio
  opcion_a <- paste0(fraccion, " \\cdot (", format(precio_original, big.mark=".", decimal.mark=","), ")")
} else {
  # Opción A incorrecta: Fracción invertida × precio
  opcion_a <- paste0(fraccion_invertida, " \\cdot (", format(precio_original, big.mark=".", decimal.mark=","), ")")
}

# Opción B
if (indice_correcto == 2) {
  # Opción B correcta: precio × porcentaje ÷ 100
  opcion_b <- paste0(format(precio_original, big.mark=".", decimal.mark=","), " \\times ", porcentaje_descuento, "\\%", " \\div 100")
} else {
  # Opción B incorrecta: precio × porcentaje × 100
  opcion_b <- paste0(format(precio_original, big.mark=".", decimal.mark=","), " \\times ", porcentaje_descuento, "\\%", " \\times 100")
}

# Opción C
if (indice_correcto == 3) {
  # Opción C correcta: precio × porcentaje / 100
  ...
  # Opción C incorrecta: precio × 100 / porcentaje
  opcion_c <- paste0("\\frac{", format(precio_original, big.mark=".", decimal.mark=","), " \\times 100}{", porcentaje_descuento, "}")
}

# Opción D: Siempre incorrecta, calcula el precio final
...
# El índice correcto ya fue seleccionado aleatoriamente en la generación de opciones
solucion <- rep(0, 4)
...
¿Cuál de las siguientes `r termino_procedimiento` permite calcular este valor?
...
Para resolver este problema, debemos analizar cada uno de los procedimientos propuestos y determinar cuál permite calcular correctamente el ahorro obtenido al aplicar un descuento del `r porcentaje_descuento`% a un artículo que cuesta $`r format(precio_original, big.mark=".", decimal.mark=",")`.
...
#### * Opción: $`r opcion_a`$
`r if(indice_correcto == 1) {
  paste0("Esta opción representa la fracción equivalente al porcentaje de descuento multiplicada por el precio original. $",
         fraccion, "$ es equivalente a ", porcentaje_descuento, "%, y al multiplicar por el precio original obtenemos el valor del descuento: $",
         fraccion, " \\times ", format(precio_original, big.mark=".", decimal.mark=","), " = ",
         format(valor_descuento, big.mark=".", decimal.mark=","), "$. **Este procedimiento SÍ permite calcular correctamente el ahorro.**")
} else {
  resultado_incorrecto_a <- precio_original * (1 / (porcentaje_descuento / 100))
  paste0("Esta opción utiliza una fracción invertida, lo que produce un resultado incorrecto. La fracción correcta para ",
         porcentaje_descuento, "% sería $", fraccion, "$, pero aquí se usa $", fraccion_invertida, "$. Al multiplicar esta fracción invertida por el precio original, obtenemos un valor incorrecto: $",
         fraccion_invertida, " \\times ", format(precio_original, big.mark=".", decimal.mark=","), " = ",
         format(resultado_incorrecto_a, big.mark=".", decimal.mark=","), "$. Este resultado no corresponde al ahorro de $",
         format(valor_descuento, big.mark=".", decimal.mark=","), "$. **Este procedimiento NO permite calcular correctamente el ahorro.**")
}`

#### * Opción: $`r opcion_b`$
`r if(indice_correcto == 2) {
  paste0("Esta opción representa el cálculo directo del porcentaje de descuento sobre el precio original. Al multiplicar el precio por el porcentaje y dividir por 100, obtenemos el valor del descuento: $",
         format(precio_original, big.mark=".", decimal.mark=","), " \\times ", porcentaje_descuento, "\\% \\div 100 = ",
         format(valor_descuento, big.mark=".", decimal.mark=","), "$. **Este procedimiento SÍ permite calcular correctamente el ahorro.**")
} else {
  resultado_incorrecto_b <- precio_original * porcentaje_descuento * 100
  paste0("Esta opción contiene un error común: multiplicar por 100 en lugar de dividir por 100. La operación correcta sería dividir por 100, pero aquí se multiplica por 100: $",
         format(precio_original, big.mark=".", decimal.mark=","), " \\times ", porcentaje_descuento, "\\% \\times 100 = ",
         format(resultado_incorrecto_b, big.mark=".", decimal.mark=","), "$. Este resultado es 10,000 veces mayor que el ahorro real de $",
         format(valor_descuento, big.mark=".", decimal.mark=","), "$. **Este procedimiento NO permite calcular correctamente el ahorro.**")
}`

#### * Opción: $`r opcion_c`$
`r if(indice_correcto == 3) {
  if (usar_fraccion_opcion_c) {
    paste0("Esta opción representa la fórmula estándar para calcular un porcentaje del precio original: $\\frac{",
           porcentaje_descuento, "}{100} \\times ", format(precio_original, big.mark=".", decimal.mark=","), " = ",
           format(valor_descuento, big.mark=".", decimal.mark=","), "$. **Este procedimiento SÍ permite calcular correctamente el ahorro.**")
  } else {
    paste0("Esta opción representa la fórmula estándar para calcular un porcentaje del precio original: $\\frac{",
           format(precio_original, big.mark=".", decimal.mark=","), " \\times ", porcentaje_descuento, "}{100} = ",
           format(valor_descuento, big.mark=".", decimal.mark=","), "$. **Este procedimiento SÍ permite calcular correctamente el ahorro.**")
  }
} else {
  resultado_incorrecto_c <- precio_original * 100 / porcentaje_descuento
  paste0("Esta opción contiene un error en la fórmula: se divide el producto del precio por 100 entre el porcentaje, en lugar de multiplicar el precio por el porcentaje y dividir por 100. La fórmula correcta sería $\\frac{",
         format(precio_original, big.mark=".", decimal.mark=","), " \\times ", porcentaje_descuento, "}{100}$, pero aquí se usa $\\frac{",
         format(precio_original, big.mark=".", decimal.mark=","), " \\times 100}{", porcentaje_descuento, "}$. Este cálculo da como resultado $",
         format(resultado_incorrecto_c, big.mark=".", decimal.mark=","), "$, que no corresponde al ahorro de $",
         format(valor_descuento, big.mark=".", decimal.mark=","), "$. **Este procedimiento NO permite calcular correctamente el ahorro.**")
}`

#### * Opción: $`r opcion_d`$
Esta opción representa el factor para calcular el precio final (después del descuento) multiplicado por el precio original. $`r format(factor_precio_final, nsmall=2, decimal.mark=",")`$ es equivalente a $(1 - `r porcentaje_descuento`/100)$. Al multiplicar este factor por el precio original, obtenemos el precio final después del descuento: $`r format(factor_precio_final, nsmall=2, decimal.mark=",")` \times `r format(precio_original, big.mark=".", decimal.mark=",")` = `r format(precio_final, big.mark=".", decimal.mark=",")`$. Este cálculo nos da el precio final después del descuento, NO el ahorro. **Este procedimiento NO permite calcular correctamente el ahorro.**

### Verificación matemática:

Para calcular correctamente el ahorro en un descuento porcentual, debemos multiplicar el precio original por el porcentaje de descuento y dividir por 100:

Ahorro = Precio original × Porcentaje de descuento ÷ 100
Ahorro = $`r format(precio_original, big.mark=".", decimal.mark=",")` × `r porcentaje_descuento`% ÷ 100 = $`r format(valor_descuento, big.mark=".", decimal.mark=",")`
