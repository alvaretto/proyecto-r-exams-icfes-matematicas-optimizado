% Plantilla específica para Pandoc con formato legal de dos columnas
\documentclass[11pt]{article}

\usepackage[utf8]{inputenc}
\usepackage{geometry}
\usepackage{color}
\usepackage{url}
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{enumitem}
\usepackage{fancyhdr}
\usepackage{titling}
\usepackage{multicol}
\usepackage{graphicx}
\usepackage{array}
\usepackage{tabularx}
\usepackage{ltxtable}
\usepackage{adjustbox}

% Configuración del idioma
\usepackage[spanish]{babel}

% Configuración de página legal (8.5" x 14")
\geometry{
  papersize={215.9mm,355.6mm},
  top=20mm,
  bottom=20mm,
  left=15mm,
  right=15mm,
}

% Configuración de dos columnas con separador visible
\setlength{\columnseprule}{0.4pt}
\setlength{\columnsep}{20pt}

% Configuración de imágenes para dos columnas
\setkeys{Gin}{width=0.85\columnwidth,keepaspectratio}

% Configuración de tablas para dos columnas - FORZAR ADAPTACIÓN
\renewcommand{\arraystretch}{1.0}
\setlength{\tabcolsep}{1pt}

% Redefinir TODOS los entornos de tabla para forzar adaptación
\let\oldtabular\tabular
\let\endoldtabular\endtabular
\renewenvironment{tabular}[1]{%
  \footnotesize
  \setlength{\tabcolsep}{1pt}
  \begin{adjustbox}{width=\columnwidth,center}
  \begin{oldtabular}{#1}
}{%
  \end{oldtabular}
  \end{adjustbox}
}

\let\oldtable\table
\let\endoldtable\endtable
\renewenvironment{table}[1][htbp]{%
  \begin{oldtable}[#1]
  \centering
  \footnotesize
}{%
  \end{oldtable}
}

\let\oldlongtable\longtable
\let\endoldlongtable\endlongtable
\renewenvironment{longtable}[1]{%
  \footnotesize
  \setlength{\tabcolsep}{1pt}
  \begin{adjustbox}{width=\columnwidth,center}
  \begin{oldlongtable}{#1}
}{%
  \end{oldlongtable}
  \end{adjustbox}
}

% Encabezado y pie de página
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Examen}
\fancyhead[R]{Página \thepage}
\fancyfoot[C]{\small I. E. Pedacito de Cielo}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\footrulewidth}{0.4pt}

% Configuración de título
\pretitle{\begin{center}\LARGE\bfseries}
\posttitle{\par\end{center}\vskip 1em}
\preauthor{\begin{center}\large\scshape}
\postauthor{\par\end{center}}
\predate{\begin{center}\large\scshape}
\postdate{\par\end{center}}

% Configuración para Pandoc
\providecommand{\tightlist}{\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}

% Redefinir algunos comandos de Pandoc para que funcionen con multicol
\let\oldquote\quote
\let\endoldquote\endquote
\renewenvironment{quote}{\footnotesize\oldquote}{\endoldquote}

% Configuración de párrafos
\setlength{\parskip}{0.5ex plus0.1ex minus0.1ex}
\setlength{\parindent}{0em}

\title{I. E. Pedacito de Cielo, La Tebaida, Quindío}
\author{Simulacro Matemáticas Saber ICFES}
\date{Probabilidad y Estadística - Formato Legal}

\begin{document}

\maketitle

% Forzar inicio de dos columnas desde el principio del contenido
\begin{multicols}{2}

\begin{enumerate}[label=\arabic*.]
#-
  \item
#-
  \textbf{Escenario}\\
#-
  ##Question##
#-
  \begin{enumerate}[label=(\alph*)]
    \item ##Questionlist##
  \end{enumerate}
#-

  \textbf{Solución}\\
#-
  ##Solution##
#-
  \begin{enumerate}[label=(\alph*)]
    \item ##Solutionlist##
  \end{enumerate}
#-
#-
\end{enumerate}

\end{multicols}

\end{document}
