...
# Script de Pruebas Estadísticas para Verificación de Aleatorización Equilibrada
# Objetivo: Verificar que la opción correcta se distribuye uniformemente entre A, B, C, D
# Criterio de éxito: 25% ± 5% para cada opción (20-30% aceptable)

library(exams)

cat("=== PRUEBAS ESTADÍSTICAS DE ALEATORIZACIÓN EQUILIBRADA ===\n\n")

# Configuración de pruebas
NUM_VERSIONES <- 100  # Muestra robusta para análisis estadístico
TOLERANCIA_PORCENTAJE <- 5  # ±5% de tolerancia respecto al 25% ideal

# Función para extraer información de aleatorización
analizar_aleatorizacion <- function(version_num) {
  tryCatch({
...
cat(sprintf("Ejecutando análisis estadístico en %d versiones...\n\n", NUM_VERSIONES))
...
for (i in 1:NUM_VERSIONES) {
  ...
  resultado <- analizar_aleatorizacion(i)
...
if (versiones_exitosas > 0) {
  ...
  cat("\nIntervalos de confianza (95%) para cada opción:\n")
  for (i in 1:4) {
    letra <- LETTERS[i]
    p_observado <- porcentajes[letra] / 100
    n <- versiones_exitosas
    
    # Intervalo de confianza binomial
    error_estandar <- sqrt(p_observado * (1 - p_observado) / n)
    margen_error <- 1.96 * error_estandar
    ic_inferior <- (p_observado - margen_error) * 100
    ic_superior <- (p_observado + margen_error) * 100
    
    # Verificar si 25% está dentro del intervalo
    contiene_25 <- ic_inferior <= 25 && 25 <= ic_superior
    estado <- if (contiene_25) "✅" else "❌"
    
    cat(sprintf("  Tabla %s: [%.1f%%, %.1f%%] %s\n", letra, ic_inferior, ic_superior, estado))
  }
  ...
  if (resultado_final) {
    cat("\nLa aleatorización cumple todos los criterios estadísticos para distribución equilibrada.\n")
  } else {
    cat("\nLa aleatorización requiere ajustes para lograr distribución perfectamente equilibrada.\n")
  }
}
