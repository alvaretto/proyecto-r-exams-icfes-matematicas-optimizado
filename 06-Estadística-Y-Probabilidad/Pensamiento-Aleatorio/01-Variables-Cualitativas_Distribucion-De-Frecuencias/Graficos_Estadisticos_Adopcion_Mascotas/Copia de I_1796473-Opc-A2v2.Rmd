...
#source("additional_unit_tests.R")
```

```{r version diversity test, echo = FALSE, results = "hide"}
#source("version_diversity_test.R")
```

```{r unit_tests, echo = FALSE, results = "hide"}
library(testthat)
...
test_that("Pruebas de validación de datos", {
  ...
  expect_length(selmascota, 3)

  # Pruebas de rango
  expect_true(all(vector_resultado >= 0 & vector_resultado <= 100))
  expect_equal(sum(vector_resultado), 100)
  expect_true(enkuestados >= 60 && enkuestados <= 600)

  # Pruebas de distribución (ejemplo)
  expect_true(mashor == max(vector_resultado))

  # Pruebas de relación entre variables
  tolerance <- 1
  expect_true(abs(maskota1 + maskota2 + maskota3 - enkuestados) < tolerance)
})