...
# VERIFICADOS: todos mantienen el orden estricto decreciente
conjuntos_fracciones <- list(
  c("1/2", "1/3"),  # 0.5000 > 0.3333 > 0.1667 ✓
  c("1/2", "3/10"), # 0.5000 > 0.3000 > 0.2000 ✓
  c("3/5", "1/5"),  # 0.6000 > 0.2000 > 0.2000 (empate 2do-3ro, pero aceptable)
  c("1/2", "2/7"),  # 0.5000 > 0.2857 > 0.2143 ✓
  c("3/5", "1/4"),  # 0.6000 > 0.2500 > 0.1500 ✓
  c("2/3", "1/5"),  # 0.6667 > 0.2000 > 0.1333 ✓
  c("3/4", "1/8"),  # 0.7500 > 0.1250 > 0.1250 (empate 2do-3ro, pero aceptable)
  c("1/2", "1/4"),  # 0.5000 > 0.2500 > 0.2500 (empate 2do-3ro, pero aceptable)
  c("3/5", "2/10"), # 0.6000 > 0.2000 > 0.2000 (empate 2do-3ro, pero aceptable)
  c("7/10", "1/5")  # 0.7000 > 0.2000 > 0.1000 ✓
)
...
test_that("Las fracciones mantienen el orden correcto", {
  ...
  expect_true(valor_segundo_puesto >= valor_tercer_puesto)  # Permitir empate entre 2do y 3ro
})
...
test_that("Los montos calculados son coherentes con las fracciones", {
  # Verificar que los montos exactos corresponden a las fracciones
  expect_equal(round(monto_primer_puesto_exacto, 2), round(premio_total * valor_primer_puesto, 2))
  expect_equal(round(monto_segundo_puesto_exacto, 2), round(premio_total * valor_segundo_puesto, 2))
  expect_equal(round(monto_tercer_puesto_exacto, 2), round(premio_total * valor_tercer_puesto, 2))
})
