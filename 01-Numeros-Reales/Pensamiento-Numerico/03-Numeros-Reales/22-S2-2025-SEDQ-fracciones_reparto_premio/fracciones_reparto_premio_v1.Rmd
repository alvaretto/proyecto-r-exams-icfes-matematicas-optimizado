...
# Generar distractores basados en errores pedagógicos comunes
# Error 1: Usar directamente la fracción del primer puesto (error conceptual común)
distractor1 <- round(premio_total * valor_primer_puesto, 1)

# Error 2: Usar directamente la fracción del segundo puesto (confundir puestos)
distractor2 <- round(premio_total * valor_segundo_puesto, 1)

# Error 3: Sumar las fracciones en lugar de restar (error de operación)
suma_fracciones <- valor_primer_puesto + valor_segundo_puesto
distractor3 <- round(premio_total * suma_fracciones, 1)

# Error 4: Usar solo una de las fracciones dadas sin calcular el resto
# Alternar entre diferentes errores para mayor variabilidad
errores_adicionales <- c(
  round(premio_total * (valor_primer_puesto - valor_segundo_puesto), 1),  # Diferencia
  round(premio_total * (valor_segundo_puesto * 2), 1),                    # Doble del segundo
  round(premio_total * (valor_tercer_puesto * 1.5), 1),                   # 150% del correcto
  round(premio_total * 0.1, 1)                                            # 10% fijo
)

# Seleccionar un error adicional si algún distractor coincide con la respuesta correcta
if (distractor1 == respuesta_correcta || distractor2 == respuesta_correcta || distractor3 == respuesta_correcta) {
  # Reemplazar el distractor problemático
  if (distractor1 == respuesta_correcta) distractor1 <- sample(errores_adicionales, 1)
  if (distractor2 == respuesta_correcta) distractor2 <- sample(errores_adicionales, 1)
  if (distractor3 == respuesta_correcta) distractor3 <- sample(errores_adicionales, 1)
}

# Asegurar que todos los distractores son diferentes entre sí y de la respuesta correcta
intentos <- 0
while (length(unique(c(respuesta_correcta, distractor1, distractor2, distractor3))) < 4 && intentos < 10) {
  if (distractor1 == respuesta_correcta || distractor1 == distractor2 || distractor1 == distractor3) {
    distractor1 <- sample(errores_adicionales, 1)
  }
  if (distractor2 == respuesta_correcta || distractor2 == distractor1 || distractor2 == distractor3) {
    distractor2 <- sample(errores_adicionales, 1)
  }
  if (distractor3 == respuesta_correcta || distractor3 == distractor1 || distractor3 == distractor2) {
    distractor3 <- sample(errores_adicionales, 1)
  }
  intentos <- intentos + 1
}
