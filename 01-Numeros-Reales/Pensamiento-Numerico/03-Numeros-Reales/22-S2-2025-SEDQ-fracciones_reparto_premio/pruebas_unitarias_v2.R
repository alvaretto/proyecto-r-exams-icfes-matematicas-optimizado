...
# =============================================================================
# Pruebas Unitarias Especializadas para fracciones_reparto_premio_v2.Rmd
# =============================================================================
# Este script realiza pruebas exhaustivas específicas para la versión 2:
# - Pregunta por el TERCER PUESTO
# - Método de reparto SECUENCIAL (fracción de fracción)
# - Validaciones específicas para reparto secuencial
# =============================================================================
...
archivo_rmd <- "fracciones_reparto_premio_v2.Rmd"
...
# Función para validar estructura de datos específica para v2
validar_estructura_datos_v2 <- function(datos) {
  errores <- character(0)

  # Variables obligatorias específicas para v2 (reparto secuencial)
  ...
  if (all(c("respuesta_correcta", "monto_tercer_puesto") %in% names(datos))) {
    if (datos$respuesta_correcta != datos$monto_tercer_puesto) {
      errores <- c(errores, "En v2, la respuesta correcta debe ser el monto del tercer puesto")
    }
  }
...
# Función para extraer datos específica para v2
extraer_datos_version_v2 <- function(simulacion_id) {
  ...
  tryCatch({
    ...
    variables_clave <- c(
      ...
      "fraccion_primer_puesto_str", "fraccion_segundo_puesto_str",
      "valor_f1", "valor_f2", "monto_primer_puesto_exacto", "monto_segundo_puesto_exacto",
      "monto_primer_puesto", "monto_segundo_puesto", "monto_tercer_puesto",
      "restante_despues_primero",  # Específico para v2
    ...
    # Añadir metadatos específicos para v2
    ...
    # Validar estructura de datos específica para v2
    errores_validacion <- validar_estructura_datos_v2(datos)
...
# =============================================================================
# FUNCIONES DE VALIDACIÓN ESPECÍFICAS PARA V2 (REPARTO SECUENCIAL)
# =============================================================================

# Verificar que los premios están en el rango correcto (30-400, múltiplos de 2)
verificar_rango_premios_v2 <- function(datos) {
  if (!("premio_total" %in% names(datos))) {
...
verificar_orden_premios_v2 <- function(datos) {
...
verificar_suma_montos_v2 <- function(datos) {
...
# Verificar cálculo secuencial específico para v2
verificar_calculo_secuencial_v2 <- function(datos) {
  variables_necesarias <- c("premio_total", "monto_primer_puesto", "restante_despues_primero",
                           "monto_segundo_puesto", "monto_tercer_puesto")
  variables_faltantes <- variables_necesarias[!variables_necesarias %in% names(datos)]

  if (length(variables_faltantes) > 0) {
    return(list(valido = FALSE, error = paste("Variables faltantes para cálculo secuencial:", paste(variables_faltantes, collapse = ", "))))
  }

  # Verificar que el restante después del primero es correcto
  restante_calculado <- datos$premio_total - datos$monto_primer_puesto
  diferencia_restante <- abs(restante_calculado - datos$restante_despues_primero)

  if (diferencia_restante > tolerancia_redondeo) {
    return(list(valido = FALSE, error = paste("Restante después del primero incorrecto. Calculado:", restante_calculado, "Almacenado:", datos$restante_despues_primero, "Diferencia:", diferencia_restante)))
  }

  # Verificar que segundo + tercero = restante después del primero
  suma_segundo_tercero <- datos$monto_segundo_puesto + datos$monto_tercer_puesto
  diferencia_suma <- abs(suma_segundo_tercero - datos$restante_despues_primero)

  if (diferencia_suma > tolerancia_redondeo) {
    return(list(valido = FALSE, error = paste("Suma segundo+tercero (", suma_segundo_tercero, ") != Restante después primero (", datos$restante_despues_primero, "), diferencia:", diferencia_suma)))
  }
...
verificar_opciones_unicas_v2 <- function(datos) {
  if (!("opciones_mezcladas" %in% names(datos))) {
...
# Verificar coherencia de género específica para v2
verificar_coherencia_genero_v2 <- function(datos) {
  errores <- character(0)
...
# Verificar validez de fracciones específica para v2 (secuencial)
verificar_fracciones_validas_v2 <- function(datos) {
  variables_fracciones <- c("valor_f1", "valor_f2")
  ...
  # En reparto secuencial, las fracciones no necesariamente suman 1
  # Verificar que todas las fracciones son positivas y menores que 1
  if (datos$valor_f1 <= 0 || datos$valor_f1 >= 1) {
    return(list(valido = FALSE, error = paste("Fracción primer puesto (", datos$valor_f1, ") debe estar entre 0 y 1")))
  }

  if (datos$valor_f2 <= 0 || datos$valor_f2 >= 1) {
    return(list(valido = FALSE, error = paste("Fracción segundo puesto (", datos$valor_f2, ") debe estar entre 0 y 1")))
  }

  # Verificar que la suma de fracciones no excede 1 (para que quede algo para el tercero)
  suma_fracciones_aproximada <- datos$valor_f1 + (datos$valor_f2 * (1 - datos$valor_f1))
  if (suma_fracciones_aproximada >= 1) {
    return(list(valido = FALSE, error = paste("Las fracciones secuenciales no dejan espacio para el tercer puesto. Suma aproximada:", suma_fracciones_aproximada)))
  }

  return(list(valido = TRUE, error = NULL))
}

# Verificar respuesta correcta específica para v2 (debe ser tercer puesto)
verificar_respuesta_correcta_v2 <- function(datos) {
  ...
  # Verificar que la respuesta correcta coincide con el monto del tercer puesto (específico para v2)
  if (datos$respuesta_correcta != datos$monto_tercer_puesto) {
    return(list(valido = FALSE, error = paste("En v2, respuesta correcta (", datos$respuesta_correcta, ") debe ser igual al monto tercer puesto (", datos$monto_tercer_puesto, ")")))
  }
...
# =============================================================================
# EJECUCIÓN PRINCIPAL DE PRUEBAS PARA V2
# =============================================================================

# Función principal para ejecutar todas las pruebas de v2
ejecutar_pruebas_v2 <- function() {
  log_mensaje("Iniciando pruebas para fracciones_reparto_premio_v2.Rmd (reparto secuencial)", "INFO")
  ...
  for (i in 1:num_simulaciones) {
    tryCatch({
      resultado <- extraer_datos_version_v2(i)
  ...
  # Ejecutar todas las pruebas específicas para v2
  log_mensaje("Ejecutando pruebas específicas para v2 (reparto secuencial)", "INFO")
  ...
  resultados_premios <- lapply(resultados_finales, verificar_rango_premios_v2)
  ...
  resultados_orden <- lapply(resultados_finales, verificar_orden_premios_v2)
  ...
  resultados_suma <- lapply(resultados_finales, verificar_suma_montos_v2)
  ...
  # PRUEBA 5: Verificar cálculo secuencial (específico para v2)
  log_mensaje("PRUEBA 5: Verificación de cálculo secuencial", "INFO")
  resultados_secuencial <- lapply(resultados_finales, verificar_calculo_secuencial_v2)
  secuencial_correcto <- sapply(resultados_secuencial, function(x) x$valido)
  num_secuencial_incorrecto <- sum(!secuencial_correcto)

  cat("Casos con cálculo secuencial incorrecto:", num_secuencial_incorrecto, "de", length(resultados_finales), "\n")

  prueba5_aprobada <- num_secuencial_incorrecto == 0
  if (prueba5_aprobada) {
    log_mensaje("APROBADO: Cálculo secuencial correcto en todos los casos", "SUCCESS")
  } else {
    log_mensaje(paste("FALLIDO:", num_secuencial_incorrecto, "casos con cálculo secuencial incorrecto"), "ERROR")
  }

  # PRUEBA 6: Verificar opciones únicas
  log_mensaje("PRUEBA 6: Verificación de opciones únicas", "INFO")
  resultados_opciones <- lapply(resultados_finales, verificar_opciones_unicas_v2)
  ...
  prueba6_aprobada <- num_opciones_duplicadas == 0
  if (prueba6_aprobada) {
  ...
  # PRUEBA 7: Verificar coherencia de género
  log_mensaje("PRUEBA 7: Verificación de coherencia de género", "INFO")
  resultados_genero <- lapply(resultados_finales, verificar_coherencia_genero_v2)
  ...
  prueba7_aprobada <- num_genero_incoherente == 0
  if (prueba7_aprobada) {
  ...
  # PRUEBA 8: Verificar fracciones válidas (secuencial)
  log_mensaje("PRUEBA 8: Verificación de fracciones válidas (secuencial)", "INFO")
  resultados_fracciones <- lapply(resultados_finales, verificar_fracciones_validas_v2)
  ...
  prueba8_aprobada <- num_fracciones_invalidas == 0
  if (prueba8_aprobada) {
    log_mensaje("APROBADO: Fracciones secuenciales válidas en todos los casos", "SUCCESS")
  } else {
    log_mensaje(paste("FALLIDO:", num_fracciones_invalidas, "casos con fracciones inválidas"), "ERROR")
  }

  # PRUEBA 9: Verificar respuesta correcta (específica para v2 - tercer puesto)
  log_mensaje("PRUEBA 9: Verificación de respuesta correcta (tercer puesto)", "INFO")
  resultados_respuesta <- lapply(resultados_finales, verificar_respuesta_correcta_v2)
  ...
  cat("REPORTE FINAL PARA V2 (REPARTO SECUENCIAL)\n")
  ...
  cat("✓ Respuesta correcta (tercer puesto):", if(prueba9_aprobada) "APROBADO" else "FALLIDO", "\n")

  if (todas_aprobadas) {
    cat("\n🎉 ¡TODAS LAS PRUEBAS APROBADAS PARA V2!\n")
    cat("✓ El ejercicio v2 (reparto secuencial) está listo para producción.\n")
    log_mensaje("Todas las pruebas de v2 completadas exitosamente", "SUCCESS")
  } else {
    cat("\n⚠️  ALGUNAS PRUEBAS FALLARON PARA V2\n")
    cat("📋 Revise los detalles arriba para identificar problemas específicos.\n")
    log_mensaje("Algunas pruebas de v2 fallaron", "ERROR")
  }

  # Guardar resultados
  timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
  archivo_reporte <- paste0("reporte_pruebas_v2_", timestamp, ".RData")
...
log_mensaje("Iniciando pruebas unitarias para v2 (reparto secuencial)", "INFO")
resultado_final <- ejecutar_pruebas_v2()

if (resultado_final) {
  log_mensaje("Pruebas unitarias v2 completadas exitosamente", "SUCCESS")
  quit(status = 0)
} else {
  log_mensaje("Pruebas unitarias v2 fallaron", "ERROR")
  quit(status = 1)
}