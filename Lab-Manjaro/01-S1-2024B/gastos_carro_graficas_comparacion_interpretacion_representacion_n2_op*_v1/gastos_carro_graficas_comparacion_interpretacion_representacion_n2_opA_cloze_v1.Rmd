...
# Aleatorización de letras de gráficas y opciones
letras_graficas <- sample(LETTERS[1:4])  # Aleatorizar A, B, C, D
nombres_graficas_base <- c(
...
# Crear mapeo de letras a tipos de gráficas
mapeo_graficas <- data.frame(
  letra = letras_graficas,
  tipo = nombres_graficas_base,
  stringsAsFactors = FALSE
)

# Aleatorizar el orden de presentación de las gráficas
orden_presentacion <- sample(1:4)
mapeo_graficas_ordenado <- mapeo_graficas[orden_presentacion, ]

# Generar opciones para la pregunta schoice final usando el MISMO orden que las gráficas mostradas
opciones_graficas <- mapeo_graficas_ordenado$tipo

# La respuesta correcta es siempre la gráfica circular por semana
respuesta_correcta_schoice <- "Circular por semana"

# Encontrar en qué posición está la respuesta correcta en el orden mostrado
indice_respuesta_correcta <- which(mapeo_graficas_ordenado$tipo == respuesta_correcta_schoice)
...
solucion_cloze <- list(
  respuesta_1,      # Paso 1: Valor del gasto objetivo (mayor o menor)
  ...
  solucion_schoice  # Paso 6: Tipo de gráfica (schoice)
)
...
test_that("Validación de coherencia matemática", {
  ...
  expect_true(all(sapply(solucion_cloze[1:5], is.numeric)))  # Solo las primeras 5 son numéricas
  expect_true(is.logical(solucion_cloze[[6]]))  # La sexta es lógica (schoice)
  expect_true(indice_respuesta_correcta >= 1 && indice_respuesta_correcta <= 4)
})

test_that("Validación de aleatorización de gráficas", {
  expect_equal(length(letras_graficas), 4)
  expect_equal(length(unique(letras_graficas)), 4)  # Todas las letras diferentes
  expect_true(all(letras_graficas %in% LETTERS[1:4]))  # Solo A, B, C, D
  expect_equal(nrow(mapeo_graficas), 4)
  expect_equal(length(opciones_graficas), 4)
  expect_equal(respuesta_correcta_schoice, "Circular por semana")  # Respuesta correcta es exactamente el tipo
  expect_true(all(opciones_graficas %in% nombres_graficas_base))  # Todas las opciones son tipos válidos
})
...
test_that("Validación de aleatorización de posiciones", {
  ...
  expect_true(all(mapeo_graficas_ordenado$tipo %in% nombres_graficas_base))  # Todos los tipos válidos
})

test_that("Validación de coherencia pedagógica específica", {
  # Verificar que la respuesta correcta sea inequívocamente única
  expect_equal(respuesta_correcta_schoice, "Circular por semana")

  # Verificar que solo hay una opción "Circular por semana" en las opciones
  count_circular_semana <- sum(opciones_graficas == "Circular por semana")
  expect_equal(count_circular_semana, 1)

  # Verificar que la respuesta correcta está en las opciones mostradas
  expect_true(respuesta_correcta_schoice %in% opciones_graficas)

  # Verificar que el índice de respuesta correcta es válido
  expect_true(indice_respuesta_correcta >= 1 && indice_respuesta_correcta <= 4)

  # Verificar que solo una opción es TRUE en solucion_schoice
  expect_equal(sum(solucion_schoice), 1)
})

test_that("Validación de diferenciación clara de distractores", {
  # Verificar que cada tipo de gráfica es único
  expect_equal(length(unique(opciones_graficas)), 4)

  # Verificar que los distractores son específicamente diferentes
  distractores_esperados <- c("Circular por categoría", "Barras apiladas por semana", "Barras agrupadas por categoría")
  distractores_presentes <- opciones_graficas[opciones_graficas != "Circular por semana"]
  expect_true(all(distractores_presentes %in% distractores_esperados))
  expect_equal(length(distractores_presentes), 3)
})

test_that("Validación de coherencia matemática completa", {
  # Verificar que la suma de gastos por categoría igual al gran total
  suma_categorias <- total_gasolina + total_parqueadero + total_peajes
  expect_equal(suma_categorias, gran_total)

  # Verificar que el porcentaje está en rango válido
  expect_true(porcentaje_objetivo >= 0 && porcentaje_objetivo <= 100)

  # Verificar que el valor objetivo corresponde realmente al mayor/menor
  if (tipo_pregunta == "mayor") {
    expect_equal(valor_objetivo, max(totales_semana))
  } else {
    expect_equal(valor_objetivo, min(totales_semana))
  }

  # Verificar precisión del cálculo de porcentaje
  porcentaje_calculado <- redondear_matematico((valor_objetivo / gran_total) * 100, 1)
  expect_equal(porcentaje_objetivo, porcentaje_calculado)
})
```
...
# Mapeo dinámico de tipos a letras usando la aleatorización real de R
mapeo_r = {")

# Generar el mapeo dinámico basado en el orden aleatorizado real
for(i in 1:4) {
  tipo <- mapeo_graficas$tipo[i]
  letra <- mapeo_graficas$letra[i]
  codigo_python_graficas <- paste0(codigo_python_graficas, "
    '", tipo, "': '", letra, "'")
  if(i < 4) {
    codigo_python_graficas <- paste0(codigo_python_graficas, ",")
  }
...
# GRÁFICA 1: Circular por categoría (SIN porcentajes automáticos)
plt.figure(figsize=(7, 7))
plt.pie(totales_categoria, labels=categorias,
        colors=colores_categorias, startangle=90)
letra = mapeo_r['Circular por categoría']
plt.title('Circular por categoría', fontsize=14, fontweight='bold')
plt.axis('equal')
plt.savefig(f'grafica_{letra.lower()}.png', dpi=200, bbox_inches='tight', transparent=False)
plt.savefig(f'grafica_{letra.lower()}.pdf', dpi=200, bbox_inches='tight', transparent=False)
plt.close()
...
letra = mapeo_r['Barras apiladas por semana']
plt.title('Barras apiladas por semana', fontsize=14, fontweight='bold')
...
# GRÁFICA 3: Circular por semana (CON porcentajes automáticos - ÚNICA RESPUESTA CORRECTA)
plt.figure(figsize=(8, 8))
plt.pie(totales_semana, labels=semanas, autopct='%1.1f%%',
        colors=colores_semanas, startangle=90,
        textprops={'fontsize': 12, 'fontweight': 'bold'},
        pctdistance=0.85, labeldistance=1.1)
letra = mapeo_r['Circular por semana']
plt.title('Circular por semana', fontsize=14, fontweight='bold')
plt.axis('equal')
plt.savefig(f'grafica_{letra.lower()}.png', dpi=200, bbox_inches='tight', transparent=False)
plt.savefig(f'grafica_{letra.lower()}.pdf', dpi=200, bbox_inches='tight', transparent=False)
plt.close()
...
plt.ylabel('Gastos (pesos)', fontweight='bold', fontsize=12)
letra = mapeo_r['Barras agrupadas por categoría']
plt.title('Barras agrupadas por categoría', fontsize=14, fontweight='bold')
...
# Mostrar las 4 gráficas en orden aleatorizado con mejor formato
for(i in 1:4) {
  letra <- mapeo_graficas_ordenado$letra[i]
  tipo <- mapeo_graficas_ordenado$tipo[i]
  archivo <- paste0("grafica_", tolower(letra), ".png")

  if (es_moodle) {
    cat(paste0("### **", tipo, "**\n\n"))
    cat(paste0("![](", archivo, "){width=70%}\n\n"))
    cat("<br>\n\n")
  } else {
    cat(paste0("### **", tipo, "**\n\n"))
    cat(paste0("![](", archivo, "){width=75%}\n\n"))
    cat("<br>\n\n")
  }
}
```
...
### Paso 6: Identificación del tipo de gráfica más apropiada (CON PUNTUACIÓN)
¿Cuál de las gráficas presentadas permite identificar DIRECTAMENTE el porcentaje que representa cada semana del total mensual?

##ANSWER6##

**Conclusión:** La semana con `r texto_pregunta` gasto representó el ##ANSWER5##% del gasto total mensual.
...
"`r respuesta_correcta_schoice`" - **ÚNICA RESPUESTA CORRECTA INEQUÍVOCA**

- **EXCLUSIVAMENTE muestra porcentajes automáticos**: Cada sector incluye automáticamente el porcentaje exacto (ej: `r formato_estandar(porcentaje_objetivo, 1)`%) sin necesidad de cálculos
- **Visualización DIRECTA de proporciones**: Los porcentajes aparecen automáticamente en cada sector, eliminando cualquier interpretación subjetiva
- **Comparación inmediata**: El tamaño visual + porcentaje numérico permiten identificación instantánea del `r texto_analisis`
- **Diseño específico para análisis porcentual**: Es la ÚNICA representación que combina visualización proporcional con valores porcentuales exactos automáticos

**Análisis detallado de distractores (por qué son INCORRECTOS):**

- **Gráfica circular por categoría**: ❌ **INCORRECTO** - Muestra proporciones de categorías (gasolina, parqueadero, peajes), NO de semanas. Imposible identificar qué semana tuvo `r texto_pregunta` gasto total.
- **Gráfica de barras apiladas por semana**: ❌ **INCORRECTO** - Aunque muestra totales por semana, NO incluye porcentajes automáticos. Requiere cálculo manual adicional para obtener proporciones.
- **Gráfica de barras agrupadas por categoría**: ❌ **INCORRECTO** - Agrupa por categoría, no por semana. No permite comparar totales semanales ni calcular proporciones por semana.
...
- Tipo de análisis: Identificación del `r texto_pregunta` valor
...
extype: cloze
exsolution: `r paste(c(solucion_cloze[1:5], mchoice2string(solucion_cloze[[6]])), collapse="|")`
