...
semana_mayor_gasto <- which.max(totales_semana)
valor_mayor_gasto <- max(totales_semana)

# Calcular porcentaje de la semana con mayor gasto
porcentaje_mayor_gasto <- redondear_matematico((valor_mayor_gasto / gran_total) * 100, 1)

# Respuestas para el formato cloze
respuesta_1 <- valor_mayor_gasto          # Paso 1: Valor mayor gasto
respuesta_2 <- gran_total                 # Paso 2: Total general
respuesta_3 <- valor_mayor_gasto          # Paso 3: Numerador de la fórmula
respuesta_4 <- gran_total                 # Paso 4: Denominador de la fórmula
respuesta_5 <- porcentaje_mayor_gasto     # Paso 5: Porcentaje final
...
# Generar opciones para la pregunta schoice final (solo el tipo, sin "Gráfica X:")
opciones_graficas <- mapeo_graficas$tipo

# La respuesta correcta es siempre la gráfica circular por semana (tipo 3)
indice_tipo_correcto <- 3  # "Circular por semana"
respuesta_correcta_schoice <- nombres_graficas_base[indice_tipo_correcto]

# Mezclar opciones aleatoriamente
orden_aleatorio <- sample(1:4)
opciones_mezcladas <- opciones_graficas[orden_aleatorio]

# Para schoice en cloze, necesitamos un vector lógico
indice_respuesta_correcta <- which(orden_aleatorio == indice_tipo_correcto)
...
test_that("Validación de datos generados", {
  ...
  expect_true(semana_mayor_gasto >= 1 && semana_mayor_gasto <= 4)
})

test_that("Validación de coherencia matemática", {
  expect_equal(respuesta_1, valor_mayor_gasto)
  expect_equal(respuesta_2, gran_total)
  expect_equal(respuesta_3, valor_mayor_gasto)
  expect_equal(respuesta_4, gran_total)
  expect_equal(respuesta_5, porcentaje_mayor_gasto)
...
plt.title('Registro Semanal de Gastos del ", vehiculo, "', fontsize=14, fontweight='bold', pad=20)
plt.savefig('tabla_gastos.png', dpi=150, bbox_inches='tight', transparent=False)
plt.savefig('tabla_gastos.pdf', dpi=150, bbox_inches='tight', transparent=False)
plt.close()
")
...
totales_categoria = [", paste(c(total_gasolina, total_parqueadero, total_peajes), collapse=", "), "]
...
plt.title('Circular por categoría', fontsize=12, fontweight='bold')
plt.axis('equal')
plt.savefig(f'grafica_{letra.lower()}.png', dpi=150, bbox_inches='tight', transparent=False)
plt.savefig(f'grafica_{letra.lower()}.pdf', dpi=150, bbox_inches='tight', transparent=False)
plt.close()
...
plt.xlabel('Semanas', fontweight='bold')
plt.ylabel('Gastos (pesos)', fontweight='bold')
letra = mapeo_r['Barras apiladas por semana']
plt.title('Barras apiladas por semana', fontsize=12, fontweight='bold')
plt.legend()
...
plt.title('Circular por semana', fontsize=12, fontweight='bold')
plt.axis('equal')
plt.savefig(f'grafica_{letra.lower()}.png', dpi=150, bbox_inches='tight', transparent=False)
plt.savefig(f'grafica_{letra.lower()}.pdf', dpi=150, bbox_inches='tight', transparent=False)
plt.close()
...
plt.title('Barras agrupadas por categoría', fontsize=12, fontweight='bold')
...
if (es_moodle) {
  cat("![](tabla_gastos.png){width=80%}")
} else {
  cat("![](tabla_gastos.png){width=90%}")
}
```
...
for(i in 1:4) {
  ...
  if (es_moodle) {
    cat(paste0("**", tipo, "**\n\n"))
    cat(paste0("![](", archivo, "){width=45%}\n\n"))
  } else {
    cat(paste0("**", tipo, "**\n\n"))
    cat(paste0("![](", archivo, "){width=48%}\n\n"))
  }
}
```

Para analizar qué semana representó el mayor porcentaje del gasto total mensual, resuelva paso a paso:
...
### Paso 1: Identificación del mayor gasto semanal
Observe la tabla y identifique cuál fue el mayor gasto semanal (total por semana).
...
- Numerador (mayor gasto semanal): $##ANSWER3##
...
### Paso 1: Identificación correcta del mayor gasto semanal ✓

**Respuesta correcta:** $`r formato_estandar(valor_mayor_gasto, 0)`
...
La semana `r semana_mayor_gasto` tuvo el mayor gasto con $`r formato_estandar(valor_mayor_gasto, 0)`.
...
**Respuestas correctas:** Numerador = `r formato_estandar(valor_mayor_gasto, 0)`, Denominador = `r formato_estandar(gran_total, 0)`

La fórmula de porcentaje requiere:

- **Numerador:** El valor observado (`r formato_estandar(valor_mayor_gasto, 0)` pesos)
...
**Respuesta correcta:** `r formato_estandar(porcentaje_mayor_gasto, 1)`%

$$\text{Porcentaje} = \frac{`r formato_estandar(valor_mayor_gasto, 0)`}{`r formato_estandar(gran_total, 0)`} \times 100\% = `r formato_estandar(porcentaje_mayor_gasto, 1)`\%$$
...
- Esta opción permite visualizar directamente qué semana representa el mayor porcentaje del total mensual
- Muestra claramente las proporciones de cada semana respecto al total
- Facilita la identificación inmediata de la semana con mayor participación porcentual
...
- Mayor gasto semanal: $`r formato_estandar(valor_mayor_gasto, 0)` (Semana `r semana_mayor_gasto`)
- Gasto total mensual: $`r formato_estandar(gran_total, 0)`
- Porcentaje representado: `r formato_estandar(porcentaje_mayor_gasto, 1)`%
...
La semana `r semana_mayor_gasto` representó el **`r formato_estandar(porcentaje_mayor_gasto, 1)`%** del gasto total mensual de `r nombre_propietario` en su `r vehiculo`.
