...
# La respuesta correcta es siempre la gráfica de barras apiladas por semana
# Es la única que muestra valores absolutos semanales necesarios para los cálculos
respuesta_correcta_schoice <- "Barras apiladas por semana"
...
test_that("Validación de aleatorización de gráficas", {
  ...
  expect_equal(length(opciones_graficas), 4)
  expect_equal(respuesta_correcta_schoice, "Barras apiladas por semana")  # Respuesta correcta es exactamente el tipo
  expect_true(all(opciones_graficas %in% nombres_graficas_base))  # Todas las opciones son tipos válidos
})

test_that("Validación de unicidad de respuesta correcta", {
  # Verificar que solo "Barras apiladas por semana" puede mostrar valores absolutos semanales
  expect_true("Barras apiladas por semana" %in% nombres_graficas_base)
  expect_true("Circular por semana" %in% nombres_graficas_base)  # Muestra porcentajes, no valores absolutos
  expect_true("Circular por categoría" %in% nombres_graficas_base)  # No muestra información semanal
  expect_true("Barras agrupadas por categoría" %in% nombres_graficas_base)  # Agrupa por categoría, no por semana
  # Solo "Barras apiladas por semana" cumple el criterio de mostrar valores absolutos semanales
  expect_equal(respuesta_correcta_schoice, "Barras apiladas por semana")
})
...
mapeo_r = {
    'Circular por categoría': '", mapeo_graficas$letra[1], "',
    'Barras apiladas por semana': '", mapeo_graficas$letra[2], "',
    'Circular por semana': '", mapeo_graficas$letra[3], "',
    'Barras agrupadas por categoría': '", mapeo_graficas$letra[4], "'
}
...
# Mostrar las 4 gráficas en orden aleatorizado (sin "Gráfica X:")
for(i in 1:4) {
...
Basándose en su análisis anterior, **seleccione qué tipo de gráfica permite identificar más fácilmente los valores absolutos de gasto por semana necesarios para realizar los cálculos de porcentajes paso a paso**:

##ANSWER6##

**Conclusión:** A través de este análisis paso a paso, determinamos que la semana con `r texto_pregunta` gasto representó el ##ANSWER5##% del total mensual, y que las gráficas de barras apiladas por semana son la herramienta más apropiada para identificar valores absolutos necesarios en cálculos de porcentajes.
...
- **Única gráfica que muestra valores absolutos semanales**: La altura de cada barra representa exactamente el total de gastos por semana
- **Facilita comparación directa de magnitudes**: Permite identificar visualmente cuál semana tuvo el `r texto_pregunta` gasto sin cálculos adicionales
- **Proporciona información base para cálculos**: Los valores absolutos mostrados son exactamente los necesarios para los pasos 1-4 del análisis
- **Soporte completo para el proceso pedagógico**: Permite que el estudiante realice el análisis paso a paso usando los datos visualizados

**Análisis de distractores:**

- **Gráfica circular por categoría**: NO muestra información semanal. Es imposible identificar valores absolutos de gasto por semana individual
- **Gráfica circular por semana**: Muestra porcentajes ya calculados, NO los valores absolutos necesarios para realizar el proceso de cálculo paso a paso
- **Gráfica de barras agrupadas por categoría**: Agrupa por categoría, NO por semana. Dificulta significativamente la identificación de totales semanales

**¿Por qué es la ÚNICA respuesta correcta?**

El ejercicio requiere específicamente **valores absolutos de gasto por semana** para realizar los cálculos de los pasos 1-4. Solo la gráfica de barras apiladas por semana proporciona esta información de manera directa y clara:

- **Paso 1**: Requiere identificar el valor absoluto del `r texto_pregunta` gasto semanal → Solo visible en barras apiladas
- **Paso 2**: Requiere el valor absoluto del total mensual → Calculable sumando las alturas de las barras
- **Pasos 3-4**: Requieren los mismos valores absolutos para la fórmula → Disponibles en barras apiladas
- **Paso 5**: El cálculo se basa en los valores absolutos identificados → Proceso completo posible solo con barras apiladas
...
- La gráfica seleccionada es la única que proporciona los valores absolutos semanales necesarios para realizar el proceso de cálculo completo
