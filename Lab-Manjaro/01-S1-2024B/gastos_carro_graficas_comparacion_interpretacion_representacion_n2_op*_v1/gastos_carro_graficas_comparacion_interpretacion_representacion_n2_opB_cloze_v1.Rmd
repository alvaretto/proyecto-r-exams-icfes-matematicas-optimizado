...
# Crear vector de totales por categoría para análisis
totales_categoria <- c(total_gasolina, total_parqueadero, total_peajes)
nombres_categoria_analisis <- categorias

if (tipo_pregunta == "mayor") {
  # Identificar categoría con mayor gasto
  categoria_objetivo_indice <- which.max(totales_categoria)
  ...
  categoria_objetivo_indice <- which.min(totales_categoria)
  valor_objetivo <- min(totales_categoria)
  texto_pregunta <- "menor"
  texto_analisis <- "menor porcentaje"
}

# Nombre de la categoría objetivo
categoria_objetivo <- nombres_categoria_analisis[categoria_objetivo_indice]
...
# La respuesta correcta es siempre la gráfica circular por categoría
respuesta_correcta_schoice <- "Circular por categoría"
...
test_that("Validación de datos generados", {
  ...
  expect_true(categoria_objetivo_indice >= 1 && categoria_objetivo_indice <= 3)
...
test_that("Validación de análisis por categorías", {
  expect_equal(length(totales_categoria), 3)
  expect_equal(sum(totales_categoria), gran_total)
  expect_true(categoria_objetivo %in% categorias)
  expect_equal(valor_objetivo, totales_categoria[categoria_objetivo_indice])
  expect_true(all(totales_categoria > 0))
})
...
test_that("Validación de aleatorización de gráficas", {
  ...
  expect_equal(length(opciones_graficas), 4)
  expect_equal(respuesta_correcta_schoice, "Circular por categoría")  # Respuesta correcta es exactamente el tipo
...
Para analizar qué categoría de gasto representó el `r texto_analisis` del gasto total mensual, resuelva paso a paso:
...
**Conclusión:** La categoría con `r texto_pregunta` gasto representó el ##ANSWER5##% del gasto total mensual, y la gráfica más apropiada para analizar la distribución por categorías es la circular por categoría.
...
La categoría "`r categoria_objetivo`" tuvo el `r texto_pregunta` gasto con $`r formato_estandar(valor_objetivo, 0)`.
...
- El `r texto_pregunta` gasto por categoría individual
- El gasto total mensual (suma de todas las categorías)
...
- Esta opción permite visualizar directamente qué categoría de gasto representa el `r texto_analisis` del total mensual
- Muestra claramente las proporciones de cada categoría respecto al total
- Facilita la identificación inmediata de la categoría con `r texto_pregunta` participación porcentual
- Es el tipo de gráfica más adecuado para comparar la distribución de gastos por categorías
...
- `r stringr::str_to_title(texto_pregunta)` gasto por categoría: $`r formato_estandar(valor_objetivo, 0)` (`r categoria_objetivo`)
...
- **Identifiquen correctamente** el `r texto_pregunta` valor por categoría y el total mensual
...
- **Identifiquen el tipo de gráfica** más apropiado para analizar la distribución por categorías
- **Consoliden su aprendizaje** mediante validación de resultados

### Conclusión

La categoría "`r categoria_objetivo`" representó el **`r formato_estandar(porcentaje_objetivo, 1)`%** del gasto total mensual de `r nombre_propietario` en su `r vehiculo`.
...
- La gráfica seleccionada es la más apropiada para analizar la distribución por categorías
