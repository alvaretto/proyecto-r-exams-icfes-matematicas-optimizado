...
### Paso 5: Calcular el monto del tercer puesto
El tercer puesto recibe lo que resta después de los dos primeros puestos:

- Monto del tercer puesto = `r premio_total` - `r round(monto_primer_puesto_exacto, 2)` - `r round(monto_segundo_puesto_exacto, 2)`
- Monto del tercer puesto = `r round(monto_tercer_puesto_exacto, 2)` millones
- Redondeando: `r monto_tercer_puesto` millones de pesos

**Nota:** Con la nueva lógica, el segundo puesto se calcula como una fracción del dinero restante después del primer puesto, lo que garantiza matemáticamente que el primer puesto siempre reciba más que el segundo, y el segundo más que el tercero.

### Verificación
Comprobemos que la suma de los tres montos es igual al premio total:

**Cálculos exactos con la nueva lógica:**
...
- Tercer puesto: `r premio_total` - `r round(monto_primer_puesto_exacto, 2)` - `r round(monto_segundo_puesto_exacto, 2)` = `r round(monto_tercer_puesto_exacto, 2)` millones
