...
# Definir puntos clave para el gráfico C (CORRECTO)
# Fase 1: Aumento constante de altura
x1 = np.array([0, tiempo_aumento])
y1 = np.array([0, altura_max])
...
Esta gráfica es la que tiene color verde y muestra exactamente las cuatro fases descritas, sin elementos adicionales.

Las demás opciones contienen elementos incorrectos:

- La gráfica con color azul no muestra la pirueta con descenso constante, sino que mantiene la altura constante después del ascenso inicial.
- La gráfica que muestra un pico hacia arriba durante la pirueta es incorrecta, ya que el piloto mencionó un descenso constante.
- La gráfica que incluye una fase adicional de elevación después de la pirueta es incorrecta, ya que esta fase no fue mencionada por el piloto.
