...
# Configuración para todos los formatos de salida (patrón de Ejemplo_01.Rmd)
...
# Configuración para chunks de Python (patrón Ejemplo_01.Rmd)
knitr::knit_engines$set(python = function(options) {
  knitr::engine_output(options, options$code, '')
})

# Asegurar que Python esté correctamente configurado (usar Sys.which como Ejemplo_01.Rmd)
...
tiempos_aumento <- as.integer(sample(seq(25, 35, by = 5), 1))
tiempos_pirueta <- as.integer(sample(seq(3, 7, by = 1), 1))
tiempos_giro <- as.integer(sample(seq(15, 25, by = 5), 1))
tiempos_aterrizaje <- as.integer(sample(seq(20, 30, by = 5), 1))

# Alturas máximas posibles (en metros)
altura_maxima <- as.integer(sample(seq(100, 200, by = 20), 1))
...
# Configuración general para las gráficas CON variables de R interpoladas
# Patrón correcto: definir variables Python al inicio del código
py_config <- paste0("
import matplotlib
matplotlib.use('Agg')
...
# Variables pasadas desde R (valores escalares)
t_aumento = ", tiempos_aumento, "
t_pirueta = ", tiempos_pirueta, "
t_giro = ", tiempos_giro, "
t_aterrizaje = ", tiempos_aterrizaje, "
h_max = ", altura_maxima, "
t_total = ", tiempo_total, "
")
...
# Definir puntos clave para la gráfica correcta
# Fase 1: Aumento constante de altura
x1 = np.array([0, t_aumento])
y1 = np.array([0, h_max])

# Fase 2: Pirueta (con pico correcto)
x2 = np.array([t_aumento, t_aumento + t_pirueta/2, t_aumento + t_pirueta])
y2 = np.array([h_max, h_max*1.5, h_max/2])

# Fase 3: Giro a altura constante
x3 = np.array([t_aumento + t_pirueta, t_aumento + t_pirueta + t_giro])
y3 = np.array([h_max/2, h_max/2])

# Fase 4: Aterrizaje (descenso constante)
x4 = np.array([t_aumento + t_pirueta + t_giro, t_total])
y4 = np.array([h_max/2, 0])
...
ax.set_xlim(0, t_total + 5)
ax.set_ylim(0, h_max * 1.8)

# Mostrar gráfico
plt.tight_layout(pad=1.0)
plt.savefig('grafica_", i, ".png', dpi=100, bbox_inches='tight')
plt.close()
")
  } 
...
x2 = np.array([t_aumento, t_aumento + t_pirueta + t_giro])
y2 = np.array([h_max, h_max])

# Fase 3: Aterrizaje (descenso constante)
x3 = np.array([t_aumento + t_pirueta + t_giro, t_total])
y3 = np.array([h_max, 0])
...
x2 = np.array([t_aumento, t_aumento + t_pirueta])
y2 = np.array([h_max, h_max/2])
...
x2 = np.array([t_aumento, t_aumento + t_pirueta])
y2 = np.array([h_max, h_max/3])

# Fase EXTRA: Elevación adicional (incorrecto)
x_extra = np.array([t_aumento + t_pirueta, t_aumento + t_pirueta + 5])
y_extra = np.array([h_max/3, h_max])

# Fase 3: Giro a altura constante
x3 = np.array([t_aumento + t_pirueta + 5, t_aumento + t_pirueta + 5 + t_giro])
y3 = np.array([h_max, h_max])

# Fase 4: Aterrizaje (descenso constante)
x4 = np.array([t_aumento + t_pirueta + 5 + t_giro, t_aumento + t_pirueta + 5 + t_giro + t_aterrizaje])
y4 = np.array([h_max, 0])
...
ax.set_ylabel('Altura (m)', fontweight='bold')
ax.grid(True, linestyle='--', alpha=0.7)
ax.set_xlim(0, t_aumento + t_pirueta + 5 + t_giro + t_aterrizaje + 5)
ax.set_ylim(0, h_max * 1.8)
...
x2 = np.array([t_aumento, t_aumento + t_pirueta/2, t_aumento + t_pirueta])
y2 = np.array([h_max, h_max*1.5, h_max])
...
t = np.linspace(t_aumento + t_pirueta, t_aumento + t_pirueta + t_giro, 100)
oscilacion = (h_max/2) * 0.2 * np.sin(10 * (t - (t_aumento + t_pirueta)))
altura_base = np.ones_like(t) * (h_max/2)
x3 = t
...
")
      ...
      py_chunks[i] <- paste0(py_config, "
# Gráfica incorrecta 6: Tiempos incorrectos en las fases
# Calcular tiempos incorrectos dentro de Python
tiempo_incorrecto1 = t_aumento / 2
pirueta_doble = t_pirueta * 2
fin_fase2 = tiempo_incorrecto1 + pirueta_doble
fin_fase3 = fin_fase2 + t_giro + (t_aumento - tiempo_incorrecto1)
...
x2 = np.array([tiempo_incorrecto1, tiempo_incorrecto1 + t_pirueta, fin_fase2])
y2 = np.array([h_max, h_max*1.5, h_max/2])

# Fase 3: Giro a altura constante (tiempo más largo para compensar)
x3 = np.array([fin_fase2, fin_fase3])
y3 = np.array([h_max/2, h_max/2])

# Fase 4: Aterrizaje (descenso constante)
x4 = np.array([fin_fase3, t_total])
y4 = np.array([h_max/2, 0])
...
# Ejecutar cada gráfica y VALIDAR que se generó el PNG
for (i in 1:4) {
  png_file <- paste0("grafica_", i, ".png")
  ...
  # Validar que el PNG se generó
  if (!file.exists(png_file)) {
    stop(paste0("ERROR CRÍTICO: No se generó ", png_file,
                "\nTipo gráfica: ", orden_graficas[i],
                "\nCódigo Python (primeros 500 chars):\n", substr(py_chunks[i], 1, 500)))
  }
}
```
...
# Siguiendo patrón de Ejemplo_01.Rmd funcional
cat("-\n")
cat("![](grafica_1.png)\n\n")
cat("-\n")
cat("![](grafica_2.png)\n\n")
