...
\begin{document}
	...
	\begin{tikzpicture}[thick, scale=1.2]
		...
		% --- Sombreado para la Imagen C ---
		% La región sombreada en la imagen C es exactamente la intersección
		% de los conjuntos "Jugo de Fresa" y "Jugo de Mora".
		\begin{scope}[on background layer]
			% Usamos 'clip' para restringir el área de dibujo al círculo de Fresa.
			\clip (F) circle (\R);
			% Luego, rellenamos el círculo de Mora dentro de esa área recortada.
			% El resultado es la región común a ambos círculos.
		...
		% --- Título ---
		\node[anchor=south, font=\bfseries\large, align=center] at (0.75, 3.3) {C. \hspace{1em} Números de encuestados= 76};
		...
		\node[font=\bfseries] at (0.6, 0.4) {7};   % Los tres (Sombreado)
