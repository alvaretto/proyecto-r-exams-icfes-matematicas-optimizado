...
\\begin{tabular}{|l|c|}
\\hline
\\scriptsize\\textbf{Animal} & \\scriptsize\\textbf{Cantidad de personas } \\\\ 
     & \\scriptsize\\textbf{interesadas en adoptar} \\\\ \\hline
...
image01 <-sprintf(image01, selmascota[1], maskota1+5, selmascota[2], maskota2, selmascota[3], maskota3)
```
...
# Labels with the sizes values - mostrar 1 decimal si es diferente de cero
def format_number(size):
    if size % 1 == 0:  # Si es número entero
        return str(int(size))
    else:  # Si tiene decimales
        return f"{size:.1f}"

pie_labels = ['{}: {}'.format(label, format_number(size)) for label, size in zip(labels, sizes2)]

# GRÁFICO OPTIMIZADO PARA DOS COLUMNAS - TAMAÑO MEJORADO
fig, ax = plt.subplots(figsize=(4.0, 3.2), tight_layout=True)
ax.pie(sizes, explode=explode, labels=pie_labels, colors=colors, shadow=True, startangle=90, textprops={'fontsize': 8}, labeldistance=0.7)
ax.axis('equal')
...
La representación que muestra correctamente la información recolectada en la encuesta es el **gráfico de torta**.

**Análisis matemático de las representaciones:**

- **Tabla**: Incorrecto. Presenta valores absolutos de personas interesadas, pero no permite visualizar fácilmente las proporciones relativas entre categorías. Además, los datos numéricos aislados dificultan la comprensión inmediata de qué animal tiene mayor preferencia y en qué magnitud.

- **Gráfico de barras verticales**: Incorrecto. Aunque muestra los porcentajes correctos en valores numéricos, utiliza una escala lineal que no representa visualmente que estos porcentajes son partes complementarias de un todo (100%). Las barras independientes no evidencian que la suma de todas las categorías debe ser el total de encuestados.

- **Gráfico de barras horizontales**: Incorrecto. Muestra valores absolutos de personas en lugar de proporciones. Matemáticamente, no permite identificar directamente qué fracción del total representa cada categoría, requiriendo cálculos mentales adicionales para determinar los porcentajes relativos.

- **Gráfico de torta**: **Correcto**. Representa matemáticamente de forma óptima datos de composición porcentual porque:
  * Cada sector es proporcional al porcentaje que representa del total
  * La suma visual de todos los sectores equivale al 100% de los encuestados
  * Permite comparar inmediatamente las magnitudes relativas entre categorías
  * Evidencia claramente que se trata de una distribución de frecuencias relativas donde cada parte es complementaria de las demás

**Gráfico de torta correcto:**
