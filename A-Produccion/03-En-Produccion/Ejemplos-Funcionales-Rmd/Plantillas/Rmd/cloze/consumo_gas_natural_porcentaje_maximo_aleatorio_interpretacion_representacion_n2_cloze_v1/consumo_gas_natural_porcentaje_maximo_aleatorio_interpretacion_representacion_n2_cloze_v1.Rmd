...
`r nombre1` y `r nombre2` viven en un `r tipo_vivienda` y comparten el pago de los gastos. En `r tolower(mes_factura)` consumieron `r formato_estandar(consumo_factura, 0)` metros cúbicos de gas natural y la factura fue de $`r formato_estandar(total_factura, 0)`, incluido el cargo fijo de $`r formato_estandar(cargo_fijo, 0)` que es el mismo todos los meses. La gráfica muestra el consumo histórico en metros cúbicos de ese servicio.
...
Un hogar puede consumir máximo `r formato_estandar(consumo_maximo, 0)` metros cúbicos en un mes. Para determinar qué porcentaje del consumo máximo posible corresponde al consumo de `r tolower(mes_pregunta)`, resuelva paso a paso:
...
**Ejemplo de formato esperado:** 0.7500 (usando punto decimal)
...
**Respuesta correcta:** `r formato_estandar(consumo_pregunta, 0)` metros cúbicos

La lectura cuidadosa del gráfico de barras muestra que en `r tolower(mes_pregunta)` se consumieron exactamente `r formato_estandar(consumo_pregunta, 0)` metros cúbicos de gas natural.

### Paso 2: Identificación del consumo máximo ✓

**Respuesta correcta:** `r formato_estandar(consumo_maximo, 0)` metros cúbicos

El enunciado establece claramente que "un hogar puede consumir máximo `r formato_estandar(consumo_maximo, 0)` metros cúbicos en un mes".

### Paso 3: Configuración correcta de la fórmula ✓

**Respuestas correctas:** Numerador = `r formato_estandar(consumo_pregunta, 0)`, Denominador = `r formato_estandar(consumo_maximo, 0)`

La fórmula de porcentaje requiere:

- **Numerador:** El valor observado (`r formato_estandar(consumo_pregunta, 0)` metros cúbicos)
- **Denominador:** El valor máximo posible (`r formato_estandar(consumo_maximo, 0)` metros cúbicos)

### Paso 4: Cálculo preciso de la división ✓

**Respuesta correcta:** `r formato_estandar(respuesta_4, 4)`

$$\frac{`r formato_estandar(consumo_pregunta, 0)`}{`r formato_estandar(consumo_maximo, 0)`} = `r formato_estandar(respuesta_4, 4)`$$
...
El consumo de `r tolower(mes_pregunta)` (`r formato_estandar(consumo_pregunta, 0)` metros cúbicos) representa el **`r formato_estandar(porcentaje_correcto, 0)`%** del consumo máximo posible (`r formato_estandar(consumo_maximo, 0)` metros cúbicos).
...
**Verificación adicional**: La factura de `r tolower(mes_factura)` ($`r formato_estandar(total_factura, 0)`) corresponde exactamente al consumo mostrado en la gráfica (`r formato_estandar(consumo_factura, 0)` m³ × $`r formato_estandar(precio_por_m3, 0)` + $`r formato_estandar(cargo_fijo, 0)` cargo fijo).
