...
# Vector de soluciones para formato cloze (solo respuestas numéricas)
solucion_cloze <- list(
...
# Todos los tipos de respuesta son numéricos
tipos_respuesta <- c("num", "num", "num", "num", "num", "num")

# Tolerancias para respuestas numéricas
tolerancias <- c(0, 0, 0, 0, 0.0001, 0)
...
test_that("Validación de datos generados", {
  ...
  expect_equal(length(solucion_cloze), 6)  # 6 respuestas cloze
  expect_equal(length(tipos_respuesta), 6)  # 6 tipos de respuesta
  expect_equal(length(tolerancias), 6)  # 6 tolerancias
...
### Verificación del proceso de razonamiento

**Resultado final:** `r porcentaje_correcto`%

El formato cloze secuencial garantiza que los estudiantes:

- **Lean cuidadosamente** el gráfico para extraer el dato correcto
- **Identifiquen explícitamente** el consumo máximo del enunciado
- **Configuren correctamente** la fórmula de porcentaje
- **Realicen los cálculos** paso a paso sin saltar etapas
- **Verifiquen la coherencia** de su resultado final
...
extype: cloze
exsolution: `r paste(solucion_cloze, collapse="|")`
