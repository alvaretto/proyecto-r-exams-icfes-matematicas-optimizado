...
# ===================================================================
# Script para validar sintaxis, funcionalidad y estrategia avanzada
...
if(funcion_cargada && exists("generar_datos")) {
  ...
  # Verificar eficiencia del algoritmo de validación
  intentos_promedio <- mean(intentos_validacion)
  ...
  cat("❌ ERROR: No se puede evaluar la estrategia avanzada\n")
}
...
if(!sintaxis_ok) problemas_criticos <- problemas_criticos + 1
if(length(chunks_faltantes) > 0) problemas_criticos <- problemas_criticos + 1
if(!funcion_cargada) problemas_criticos <- problemas_criticos + 1
...
if(problemas_criticos == 0) {
  ...
  cat("\n🏆 RESULTADO: APROBADO - CÓDIGO R FUNCIONAL\n")
} else {
...
if(dir.exists("test_sintaxis_output")) {
  unlink("test_sintaxis_output", recursive = TRUE)
}
