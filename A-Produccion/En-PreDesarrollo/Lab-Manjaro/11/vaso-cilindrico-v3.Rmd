...
# La respuesta correcta es la opción 3 (perímetro de la tapa)
solucion <- 3
...
cilindro_tikz <- paste0("\\begin{tikzpicture}[>=latex]
    ...
    \\node[align=center, font=\\large] at (0,-2)
        {$2 \\times \\pi \\times ", radio, " = ",
            perimetro_base_redondeado, "$};
\\end{tikzpicture}")
```
...
$$2 \times \pi \times `r radio` = `r perimetro_base_redondeado`$$
...
Para resolver este problema, debemos identificar qué cálculo se está realizando con la fórmula $2 \times \pi \times `r radio` = `r perimetro_base_redondeado`$.
...
$P = 2 \times \pi \times `r radio` = `r perimetro_base_redondeado` \text{ cm}$
