...
Un profesor tiene una lata de forma cilíndrica. La lata tiene una base
...
\draw[<->, red] (0, \altura + 0.5) -- (\radio, \altura + 0.5) node[midway, above, font=\small, transform shape=false] {5 cm};

% Etiqueta de la altura
\draw[<->, blue] (\radio + 0.6, 0) -- (\radio + 0.6, \altura) node[midway, right, font=\small, transform shape=false] {9 cm};
...
\tightlist
\item
  Al área de la tapa de la lata.
\item
  Al área lateral de la lata.
\item
  Al perímetro de la tapa de la lata.
\item
  Al volumen de la lata.
\end{itemize}
...
\textbf{volumen de la lata}, medido en centímetros cúbicos (cm³).
...
exsolution: 0001 exshuffle: TRUE exsection:
