...
for (ancho in 3:6) for (alto in 1:2) for (x_min in 1:(grid_max - ancho)) {
  x_max <- x_min + ancho
  y_pool <- 1L:(grid_max - alto)
  y_pool <- setdiff(y_pool, x_min); y_pool <- y_pool[y_pool + alto != x_max]
  y_pool <- setdiff(y_pool, x_max); y_pool <- y_pool[y_pool + alto != x_min]
  for (y_min in y_pool) {
...
stopifnot(mal == 0L)   # 222 combinaciones, 0 desajustes (medido 2026-07-28)
```
