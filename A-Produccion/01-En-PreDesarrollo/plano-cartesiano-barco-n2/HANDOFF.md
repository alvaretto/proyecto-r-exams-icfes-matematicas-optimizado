...
| Un agente "suaviza" `prof()` para que el barco se vea mejor → la clave pasa a ser falsa sin error de sintaxis | **`tests/testthat/test_barco_bbox_invariante.R`** (guarda automática desde 2026-07-28: extrae `dibujar_barco()` del `.Rmd` real y compara el bbox con la clave en las 318 combinaciones; enganchado al runner con `watch`. **Ojo:** el pre-push corre en modo *quick* y su detección de cambios está degradada — `git lfs pre-push` consume el stdin de refs antes del `while read` del hook —, así que puede saltarla; ejecutarla a mano o con `R_TESTS_FULL=1` tras tocar `dibujar_barco()`. Ver `.claude/rules/barco-parametrico.md`) + `.claude/rules/barco-parametrico.md` (contrato C1-C3) + comentario de la invariante I-2 en el propio `.Rmd` |
