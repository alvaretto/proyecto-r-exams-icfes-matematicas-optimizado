...
semilla (R RNG)
    │
    ├─► n ← safe_sample(N_POOL, 1L)   N_POOL = {4, 5, 6}            líneas 47-48
    │      (rango medido por enumeración exhaustiva, §2)
    │
    ├─► correcta_val = n!                                            línea 53
    ├─► desarrollo (texto "n × (n-1) × ... × 1" para la Solution)     línea 57
    │
    ├─► errores_conceptuales (pool de 5, con precondicion/calcula)   líneas 63-162
    │      ├─► EST-PER-01  calcula(n) = n^(n-1)                      línea 83
    │      ├─► EST-PER-02  calcula(n) = n*n                          línea 102
    │      ├─► EST-PER-03  calcula(n) = n                            línea 120
    │      ├─► EST-PER-04  calcula(n) = (n-1)!                       línea 139
    │      └─► EST-PER-05  calcula(n) = n*(n+1)/2                    línea 160
    │
    ├─► ctx ← 1 de 6 contextos narrativos (canónico + 5 variantes)    líneas 173-244
    ├─► texto_enunciado, texto_pregunta ← ctx$enunciado/pregunta(n)   líneas 248-249
    │
    ├─► aplicables ← filtrado por precondicion (los 5 aplican siempre) líneas 263-267
    ├─► es_canonica ← (ctx_idx==1 && n==4)                            línea 269
    ├─► sel ← 3 oficiales SI es_canonica, si no sample(aplicables,3)   líneas 270-277  [Decisión D3]
    ├─► vals ← calcula(n) de los 3 errores seleccionados               línea 280
    │      ├─► stopifnot I-1..I-4 (unicidad, ≠correcta, ratio≤15x, enteros>0) líneas 285-294
    │      └─► stopifnot I-6 (canónica ⇒ opciones = {24,64,16,4})     líneas 296-299
    │
    ├─► perm ← sample(4L); opciones ← mezcla; sol ← posición correcta líneas 309-312
    │      └─► stopifnot I-5 (sol coincide con correcta, 4 únicas)    líneas 315-317
    │
    ├─► reflexion ← 1 de 6 reflexiones metacognitivas                 líneas 320-337
    └─► errores_info (codigo/nombre/texto/diagnostico/causa_raiz)     líneas 340-348
```
...
**Rango adoptado: `n ∈ {4, 5, 6}`**, codificado en `N_POOL <- c(4L, 5L, 6L)` (línea 47). El umbral
de plausibilidad de magnitud se fijó en **15×** (`stopifnot(max(all_vals) / correcta_val <= 15)`,
línea 142, regla #22 patrón P5: un distractor cuya magnitud es órdenes superior a la clave se
...
1. **En tiempo de generación** (`.Rmd`, líneas 285-299): los `stopifnot` I-1 a I-4 (sobre la terna
...
2. **Fuera del render**, por enumeración exhaustiva de las 30 combinaciones en
   `verificar_render.R` (`V6`, líneas 127-165): para cada `n ∈ {4,5,6}` recorre las 10 ternas
...
tiene ninguna: el enunciado (`.Rmd`, líneas 288-291) es texto puro y las opciones (líneas 295-299)
...
### 4.4 `exshuffle: TRUE` (línea 394) — y por qué aquí sí
...
opciones son de texto y la Solution las identifica por **contenido** (línea 307: `"La respuesta
correcta es la opción que indica **", correcta, "**"`) y por **código de error** (línea 325:
`"**", e$codigo, " — ", e$nombre, ...`) — cumple la regla #19 (letter-independence) sin necesidad
de desactivar el barajado.

El `sample(4L)` de la línea 239 no contradice `exshuffle: TRUE`: R/exams reordena a la vez
...
(líneas 317-329), que recorre `errores_info` y emite código, nombre, valor y diagnóstico de cada
...
`safe_sample()` —`n <- safe_sample(N_POOL, 1L)` (línea 48, `N_POOL` longitud 3), `ctx_idx <-
safe_sample(seq_along(contextos), 1L)` (línea 225, longitud 6) y `reflexion <-
safe_sample(reflexiones, 1L)` (línea 267, longitud 6)— actúan siempre sobre vectores de longitud
...
declarado en el comentario de cabecera del chunk (línea 21: "Familias 1 y 5 de la regla #21") y
porque no tiene costo: si en el futuro `N_POOL` se redujera a un solo valor, el código seguiría
siendo seguro sin cambios.

`ex_uid` (línea 36) usa `safe_sample(c(letters, 0:9), 8L, replace = TRUE)` con un propósito
...
El contexto 1 (`.Rmd`, líneas 174-185) no es una plantilla narrativa más: es la reproducción
...
pool de 5 (`.Rmd`, línea 274). Pero cuando la versión es la **instancia canónica** — contexto 1 (el
verbatim de `MAT-2026-1-004`) **y** `n == 4` simultáneamente — se fuerzan los tres errores
oficiales (`CODIGOS_OFICIALES <- c("EST-PER-01", "EST-PER-02", "EST-PER-03")`, línea 260;
condicional `es_canonica`, líneas 269-275). Así conviven la regla #1 (pool ampliado, variedad de
...
dentro de `if (es_canonica)` (`.Rmd`, líneas 296-299) — si alguna edición futura rompiera la
...
|---|---|---|---|
| **I-1** | Las 4 opciones son distintas: `length(unique(all_vals)) == 4L` | Previene `ERR_ANS_C` (opciones duplicadas) | `stopifnot` línea 285; 0 colisiones en las 30 ternas exhaustivas (§3) |
| **I-2** | Ningún distractor coincide con la respuesta correcta | Un distractor igual a la clave no discrimina nada | `stopifnot` línea 288 |
| **I-3** | Plausibilidad de magnitud: `max(all_vals) / correcta_val <= 15` | Regla #22, patrón P5: un distractor *outlier* de magnitud se descarta sin razonar | `stopifnot` línea 291; rango observado ahora `[1,0×, 10,8×]` (§3), umbral con holgura |
| **I-4** | Las 4 opciones son enteros positivos | Coherencia de tipo con la pregunta ("¿de cuántas formas...?") | `stopifnot` línea 294 |
| **I-5** | Exactamente una opción marcada y coincide con `n!` | Correctitud de `exsolution` (Nivel 5B de `validar_coherencia_matematica.R`) | `stopifnot` líneas 315-317 |
| **I-6** (nueva, 2026-07-29) | En la instancia canónica (`ctx_idx==1` y `n==4`), el conjunto de opciones es exactamente `{24, 64, 16, 4}` | Fidelidad al ítem oficial (OE1) cuando la Decisión D3 fuerza los 3 errores oficiales | `stopifnot` líneas 296-299 (dentro de `if (es_canonica)`), ver §4.8 |
...
| **I-9** (antes I-8) | Las **cinco** `calcula()` del pool son funciones puras (sin `sample`/`runif`/`rnorm`) | Capa D de `validar_coherencia_matematica.R`: `calcula()` debe ser determinista | `calcula()` en líneas 83, 102, 120, 139, 160 — puramente aritméticas sobre `n` |

**Nota de numeración (2026-07-29):** el `.Rmd` etiqueta en sus propios comentarios una invariante
`I-6` (líneas 296-299, la de la instancia canónica) que colisionaba con la numeración anterior de
...
## 6. Verificación de las citas de línea (2026-07-29, tras la auditoría adversarial)

Todas las referencias de línea de este documento se comprobaron contra el archivo real (481
líneas) con `grep -n` / lectura directa, después de que la auditoría adversarial del 2026-07-29
amplió el pool de errores de 3 a 5 y desplazó buena parte del archivo.

| Línea(s) | Contenido real |
|---|---|
| 22 | `pick_int <- function(a, b) if (a >= b) a else sample(a:b, 1L)` |
| 24-29 | `safe_sample <- function(x, size = 1L, replace = FALSE) { ... }` |
| 47 | `N_POOL <- c(4L, 5L, 6L)` |
| 48 | `n <- safe_sample(N_POOL, 1L)` |
| 53 | `correcta_val <- as.integer(factorial(n))` |
| 63-162 | pool de **5** errores conceptuales (`EST-PER-01` a `05`) |
| 83 | `calcula = function(n) as.integer(n^(n - 1L))` (`EST-PER-01`) |
| 102 | `calcula = function(n) as.integer(n * n)` (`EST-PER-02`) |
| 120 | `calcula = function(n) as.integer(n)` (`EST-PER-03`) |
| 139 | `calcula = function(n) as.integer(factorial(n - 1L))` (`EST-PER-04`, nuevo) |
| 160 | `calcula = function(n) as.integer(n * (n + 1L) / 2L)` (`EST-PER-05`, nuevo) |
| 173-244 | pool de 6 contextos narrativos (regla #11) |
| 246 | `ctx_idx <- safe_sample(seq_along(contextos), 1L)` |
| 260 | `CODIGOS_OFICIALES <- c("EST-PER-01", "EST-PER-02", "EST-PER-03")` (Decisión D3) |
| 263-267 | filtrado por `precondicion` (patrón genérico, regla #8) |
| 269-275 | `es_canonica`; selección de 3 errores (o de los 3 oficiales si es canónica — Decisión D3) |
| 285 | `stopifnot(length(unique(all_vals)) == 4L)` (I-1) |
| 288 | `stopifnot(!any(unname(vals) == correcta_val))` (I-2) |
| 291 | `stopifnot(max(all_vals) / correcta_val <= 15)` (I-3) |
| 294 | `stopifnot(all(all_vals > 0L), all(all_vals == as.integer(all_vals)))` (I-4) |
| 296-299 | `if (es_canonica) stopifnot(setequal(all_vals, c(24L, 64L, 16L, 4L)))` (I-6, nueva) |
| 309 | `perm     <- sample(4L)` |
| 315-317 | `stopifnot(sum(sol) == 1L, ...)`; `stopifnot(identical(opciones[which(sol == 1L)], correcta))`; `stopifnot(length(unique(opciones)) == 4L)` (I-5) |
| 337 | `reflexion <- safe_sample(reflexiones, 1L)` |
| 355 | `\makeatletter\@ifundefined{c@none}{\newcounter{none}}{}\makeatother` |
| 384-386 | comentario: regla #19, opciones identificadas por contenido y código, nunca por letra |
| 394-399 | bucle sobre `errores_info` (Análisis de cada opción) |
| 429-446 | *Caso específico* — transferencia al ítem espejo `MAT-2026-1-029`; puntero condicional a `EST-PER-01` en líneas 442-446 |
| 470 | `exshuffle: TRUE` |
