...
# Usar múltiples fuentes de aleatoriedad para garantizar variación
# Incluir un componente adicional basado en hash del tiempo actual
tiempo_actual <- Sys.time()
semilla_base <- as.numeric(tiempo_actual)
semilla_micro <- as.numeric(format(tiempo_actual, "%OS6")) * 1000000
semilla_proceso <- Sys.getpid()
semilla_hash <- sum(utf8ToInt(as.character(tiempo_actual)))
semilla_final <- (semilla_base + semilla_micro + semilla_proceso + semilla_hash + runif(1, 1, 10000)) %% .Machine$integer.max
set.seed(semilla_final)

# Forzar una nueva aleatorización adicional
invisible(sample(1:1000, 1))
...
# Función para generar título con género correcto
generar_titulo_gastos <- function(vehiculo) {
  if (vehiculo == "motocicleta") {
    return("Gastos de la motocicleta")
  } else {
    return(paste("Gastos del", vehiculo))
  }
}

titulo_gastos <- generar_titulo_gastos(vehiculo)
...
# Calcular totales por categoría para el análisis
totales_categoria <- c(total_gasolina, total_parqueadero, total_peajes)
nombres_categoria <- categorias

# Aleatorizar si se pregunta por mayor o menor gasto (ahora por categoría)
tipo_pregunta <- sample(c("mayor", "menor"), 1)

if (tipo_pregunta == "mayor") {
  # Identificar categoría con mayor gasto
  categoria_objetivo <- which.max(totales_categoria)
  valor_objetivo <- max(totales_categoria)
  texto_pregunta <- "mayor"
  texto_analisis <- "mayor porcentaje"
  nombre_categoria_objetivo <- nombres_categoria[categoria_objetivo]
} else {
  # Identificar categoría con menor gasto
  categoria_objetivo <- which.min(totales_categoria)
  valor_objetivo <- min(totales_categoria)
  texto_pregunta <- "menor"
  texto_analisis <- "menor porcentaje"
  nombre_categoria_objetivo <- nombres_categoria[categoria_objetivo]
}

# Calcular porcentaje de la categoría objetivo
...
test_that("Validación de datos generados", {
  ...
  expect_true(categoria_objetivo >= 1 && categoria_objetivo <= 3)
...
test_that("Validación de aleatorización de gráficas", {
  ...
  expect_true("circular_categoria" %in% mapeo_graficas$tipo_grafica)  # La gráfica correcta está presente
...
# Crear figura para la tabla (tamaño optimizado para Moodle)
...
# GRÁFICA 1: Circular por categoría (optimizada para Moodle Windows)
plt.figure(figsize=(5, 5))
plt.pie(totales_categoria, labels=categorias, autopct='%1.1f%%',
        colors=colores_categorias, startangle=90, textprops={'fontsize': 10, 'fontweight': 'bold'})
plt.title('", titulo_gastos, "', fontsize=12, fontweight='bold', pad=10)
plt.axis('equal')
plt.tight_layout()
plt.savefig('grafica_circular_categoria.png', dpi=150, bbox_inches='tight', transparent=False, facecolor='white')
plt.savefig('grafica_circular_categoria.pdf', dpi=150, bbox_inches='tight', transparent=False, facecolor='white')
plt.close()

# GRÁFICA 2: Barras apiladas por semana (optimizada para Moodle Windows)
plt.figure(figsize=(7, 5))
bottom = np.zeros(4)
for i, categoria in enumerate(categorias):
    plt.bar(semanas, gastos_por_categoria[categoria], bottom=bottom,
            label=categoria, color=colores_categorias[i], edgecolor='white', linewidth=1)
    bottom += gastos_por_categoria[categoria]

plt.ylabel('Gastos (pesos)', fontweight='bold', fontsize=10)
plt.xlabel('Semanas', fontweight='bold', fontsize=10)
plt.title('", titulo_gastos, "', fontsize=12, fontweight='bold', pad=10)
...
plt.tight_layout()
plt.savefig('grafica_barras_apiladas.png', dpi=150, bbox_inches='tight', transparent=False, facecolor='white')
plt.savefig('grafica_barras_apiladas.pdf', dpi=150, bbox_inches='tight', transparent=False, facecolor='white')
plt.close()

# GRÁFICA 3: Circular por semana (optimizada para Moodle Windows)
plt.figure(figsize=(5, 5))
plt.pie(totales_semana, labels=semanas, autopct='%1.1f%%',
        colors=colores_semanas, startangle=90, textprops={'fontsize': 10, 'fontweight': 'bold'})
plt.title('", titulo_gastos, "', fontsize=12, fontweight='bold', pad=10)
plt.axis('equal')
plt.tight_layout()
plt.savefig('grafica_circular_semana.png', dpi=150, bbox_inches='tight', transparent=False, facecolor='white')
plt.savefig('grafica_circular_semana.pdf', dpi=150, bbox_inches='tight', transparent=False, facecolor='white')
plt.close()

# GRÁFICA 4: Barras agrupadas por categoría (optimizada para Moodle Windows)
plt.figure(figsize=(7, 5))
...
plt.ylabel('Gastos (pesos)', fontweight='bold', fontsize=10)
plt.xlabel('Categorías', fontweight='bold', fontsize=10)
plt.title('", titulo_gastos, "', fontsize=12, fontweight='bold', pad=10)
plt.xticks(x + width*1.5, categorias, fontsize=9)
plt.yticks(fontsize=9)
plt.legend(fontsize=9, loc='upper right')
plt.grid(True, axis='y', alpha=0.3, linestyle='--')
plt.tight_layout()
plt.savefig('grafica_barras_agrupadas.png', dpi=150, bbox_inches='tight', transparent=False, facecolor='white')
plt.savefig('grafica_barras_agrupadas.pdf', dpi=150, bbox_inches='tight', transparent=False, facecolor='white')
plt.close()
...
# Tamaño optimizado para Windows 10 en Moodle
if (es_moodle) {
  cat("![](tabla_gastos.png){width=300px max-width=100%}")
} else {
...
for(i in 1:4) {
  ...
  if (es_moodle) {
    # Tamaño optimizado para Windows 10 en Moodle
    cat(paste0("![](", archivo, "){width=250px max-width=100%}\n\n"))
  } else {
...
Para analizar qué categoría representó el `r texto_analisis` del gasto total mensual, resuelva paso a paso:
...
### Paso 1: Identificación del `r texto_pregunta` gasto por categoría
Observe la tabla y identifique cuál fue el `r texto_pregunta` gasto por categoría (total por categoría).
...
- Numerador (`r texto_pregunta` gasto por categoría): $##ANSWER3##
...
Basándose en su análisis anterior, **seleccione qué tipo de gráfica permite visualizar directamente las proporciones porcentuales de cada categoría de gasto respecto al total mensual**:

##ANSWER6##

**Conclusión:** La categoría con `r texto_pregunta` gasto representó el ##ANSWER5##% del gasto total mensual.
...
### Paso 1: Identificación correcta del `r texto_pregunta` gasto por categoría ✓

**Respuesta correcta:** $`r formato_estandar(valor_objetivo, 0)`

**Análisis de gastos por categoría:**

- `r categorias[1]`: $`r formato_estandar(total_gasolina, 0)`
- `r categorias[2]`: $`r formato_estandar(total_parqueadero, 0)`
- `r categorias[3]`: $`r formato_estandar(total_peajes, 0)`

La categoría `r nombre_categoria_objetivo` tuvo el `r texto_pregunta` gasto con $`r formato_estandar(valor_objetivo, 0)`.
...
"`r respuesta_correcta_schoice`: Gráfica circular por categoría"

- **ÚNICA opción que muestra proporciones porcentuales directamente**: Los porcentajes aparecen automáticamente en cada sector del círculo
- **Visualización inmediata de partes del todo**: Cada categoría se representa como una porción del círculo completo (100%)
- **Comparación visual intuitiva**: El tamaño de cada sector es proporcional al porcentaje que representa
- **Respuesta directa a la pregunta**: Es la única que permite "visualizar directamente las proporciones porcentuales"

**Análisis de distractores:**

- **Gráfica circular por semana**: Muestra proporciones de semanas, NO de categorías de gasto
- **Gráfica de barras apiladas por semana**: Muestra composición por semana pero NO proporciones porcentuales del total
- **Gráfica de barras agrupadas por categoría**: Permite comparar valores absolutos entre categorías pero NO muestra proporciones porcentuales respecto al total

### Verificación del proceso de razonamiento completo

**Datos del problema:**

- `r stringr::str_to_title(texto_pregunta)` gasto por categoría: $`r formato_estandar(valor_objetivo, 0)` (`r nombre_categoria_objetivo`)
...
La categoría con `r texto_pregunta` gasto representó el **`r formato_estandar(porcentaje_objetivo, 1)`%** del gasto total mensual de `r nombre_propietario` en su `r vehiculo`.
