...
for (s in seq_len(N)) {
  ...
  if (e$resp_p2 != 60L %/% e$pr$tasa)  errs <- c(errs, sprintf("s%d A5 p2", s))
  if (e$resp_p3 != e$resp_p2 * e$pr$H) errs <- c(errs, sprintf("s%d A6 p3", s))
  ## v2: la guarda de disjunción es LA defensa del desacople. Si colisiona, la
  ## fuga que esta versión cierra vuelve por la puerta de atrás.
  if (length(intersect(c(e$pr$tasa, e$pr$H, e$pr$vel, e$pr$cota, e$pr$R),
                       c(e$tasa, e$H, e$vel, e$cota, e$reporte))) > 0L)
    errs <- c(errs, sprintf("s%d A6b COLISION practica/oficial", s))
