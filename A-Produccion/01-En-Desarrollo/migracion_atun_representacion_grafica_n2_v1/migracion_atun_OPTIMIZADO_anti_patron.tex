...
Una vez inicia la migración del caballa en la costa del Pacífico, su
...
correctamente la función cuadrática \(P = -d^2 + 11d + -10\).
...
\item
  ...
  \(d = \frac{-b}{2a} = \frac{11}{2} = 5.5\) días
\item
  \textbf{Valor máximo}: \(P_{max} \approx 20.2\) toneladas
\end{enumerate}
...
\textbf{Verificación con punto del vértice:}

Para \(d = 5.5\): \(P \approx 20.2\) toneladas (valor máximo)
...
\tightlist
\item
  \textbf{Falso}. Esta gráfica muestra una parábola invertida, pero el
  vértice (máximo) no está en la posición correcta (\(d \neq 5.5\)).
\item
  \textbf{Verdadero}. Esta gráfica muestra correctamente una parábola
  invertida (forma de U invertida) con máximo alrededor del día 5.5,
  conforme a la función \(P = -d^2 + 11d + -10\). Los valores aumentan
  hasta el vértice y luego disminuyen.
\item
  \textbf{Falso}. Esta gráfica muestra una parábola invertida, pero los
  valores de pesca son menores a los esperados según la función dada.
\item
  \textbf{Falso}. Esta gráfica muestra una parábola que abre hacia
  arriba (valores disminuyen al centro y aumentan en extremos),
  contrario al comportamiento esperado con coeficiente negativo
  \(-d^2\).
\end{itemize}
...
OPTIMIZADO extype: schoice exsolution: 0100 exshuffle: TRUE exsection:
